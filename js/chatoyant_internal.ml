...
module Js_fake_provider : Chatoyant_js_base.Provider.Provider.CHAT = struct
  ...
  let generate messages _options =
    if messages = [] then
      Error
        (Chatoyant_js_base.Provider.Provider.Runtime_error "missing messages")
    else
      Ok
        {
...
module Js_session =
  Chatoyant_js_base.Core.Session.Make (Js_fake_provider) (Js_clock)

module Js_shortcuts =
  Chatoyant_js_base.Core.Shortcuts.Make (Js_fake_provider) (Js_clock)
...
let provider_error_to_string = function
  | Chatoyant_js_base.Provider.Provider.Missing_api_key { provider; env_key } ->
      ...
      ^ " (" ^ env_key ^ ")"
  | Http_error { status; body } -> "HTTP " ^ string_of_int status ^ ": " ^ body
...
let ok_generation state result =
  json_object_to_string
    [
      ("ok", Chatoyant_js_base.Runtime.Json.Bool true);
      ( "state",
        Chatoyant_js_base.Runtime.Json.to_string (Js_session.to_json state)
        |> json_string );
...
let error_response message =
  json_object_to_string
    [
      ("ok", Chatoyant_js_base.Runtime.Json.Bool false);
        ...
        |> List.filter_map (function
          | Chatoyant_js_base.Runtime.Json.Object fields -> (
              ...
              | Some uri, Some schema ->
                  Some
                    Chatoyant_js_base.Schema.Json_schema.Resolver.
                      { uri; schema }
              | _ -> None)
          | _ -> None)
    | _ -> []
...
let json_schema_validate schema_text data_text options_text =
  match
    ( Chatoyant_js_base.Runtime.Json.parse schema_text,
      Chatoyant_js_base.Runtime.Json.parse data_text )
  with
  ...
  | Ok schema, Ok data -> (
      ...
      match
        Chatoyant_js_base.Schema.Json_schema.validate_json ~options schema data
      with
      | Error error -> json_schema_error_result "schema" error.message
      | Ok result ->
          Chatoyant_js_base.Schema.Json_schema.Output.to_json result
          |> Chatoyant_js_base.Runtime.Json.to_string)
...
let json_schema_project_openai_strict schema_text =
  ...
  | Ok json -> (
      ...
      | Ok schema ->
          let projected =
            Chatoyant_js_base.Schema.Json_schema.Project.openai_strict schema
          in
          projected.schema |> Chatoyant_js_base.Schema.Json_schema.to_json
          |> Chatoyant_js_base.Runtime.Json.to_string)
...
let js_chat_create_state config_json =
  ...
  let model =
    Option.value (json_string_field "model" config) ~default:"gpt-4o"
  in
  Js_session.create ~model ()
  |> Js_session.to_json |> Chatoyant_js_base.Runtime.Json.to_string
...
let create_tool : js_function =
  [%mel.raw
    {| function createTool(definition) { return new Tool(definition); } |}]
[@@mel.as "createTool"]

let merge_options : js_function =
  [%mel.raw
    {| function mergeOptions(defaults = {}, overrides = {}) { return { ...(defaults || {}), ...(overrides || {}) }; } |}]
[@@mel.as "mergeOptions"]
...
let providers_registry : js_value =
  [%mel.raw {| compatibility_surface.PROVIDERS |}]
[@@mel.as "PROVIDERS"]

let provider_ids_value : js_value =
  [%mel.raw {| compatibility_surface.PROVIDER_IDS |}]
[@@mel.as "PROVIDER_IDS"]

let openai_models_value : js_value =
  [%mel.raw {| compatibility_surface.OPENAI_MODELS |}]
[@@mel.as "OPENAI_MODELS"]

let anthropic_models_value : js_value =
  [%mel.raw {| compatibility_surface.ANTHROPIC_MODELS |}]
[@@mel.as "ANTHROPIC_MODELS"]

let xai_models_value : js_value =
  [%mel.raw {| compatibility_surface.XAI_MODELS |}]
[@@mel.as "XAI_MODELS"]

let models_by_provider_value : js_value =
  [%mel.raw {| compatibility_surface.MODELS_BY_PROVIDER |}]
[@@mel.as "MODELS_BY_PROVIDER"]

let pricing_value : js_value = [%mel.raw {| compatibility_surface.PRICING |}]
[@@mel.as "PRICING"]

let context_windows_value : js_value =
  [%mel.raw {| compatibility_surface.CONTEXT_WINDOWS |}]
[@@mel.as "CONTEXT_WINDOWS"]

let token_ratios_value : js_value =
  [%mel.raw {| compatibility_surface.TOKEN_RATIOS |}]
[@@mel.as "TOKEN_RATIOS"]

let is_provider_active_fn : js_function =
  [%mel.raw {| compatibility_surface.isProviderActive |}]
[@@mel.as "isProviderActive"]

let active_providers_fn : js_function =
  [%mel.raw {| compatibility_surface.activeProviders |}]
[@@mel.as "activeProviders"]

let detect_provider_by_model_fn : js_function =
  [%mel.raw {| compatibility_surface.detectProviderByModel |}]
[@@mel.as "detectProviderByModel"]

let assert_provider_active_fn : js_function =
  [%mel.raw {| compatibility_surface.assertProviderActive |}]
[@@mel.as "assertProviderActive"]

let get_api_key_fn : js_function =
  [%mel.raw {| compatibility_surface.getApiKey |}]
[@@mel.as "getApiKey"]

let get_base_url_fn : js_function =
  [%mel.raw {| compatibility_surface.getBaseUrl |}]
[@@mel.as "getBaseUrl"]

let resolve_provider_fn : js_function =
  [%mel.raw {| compatibility_surface.resolveProvider |}]
[@@mel.as "resolveProvider"]

let get_models_for_provider_fn : js_function =
  [%mel.raw {| compatibility_surface.getModelsForProvider |}]
[@@mel.as "getModelsForProvider"]

let get_all_known_models_fn : js_function =
  [%mel.raw {| compatibility_surface.getAllKnownModels |}]
[@@mel.as "getAllKnownModels"]

let is_known_model_fn : js_function =
  [%mel.raw {| compatibility_surface.isKnownModel |}]
[@@mel.as "isKnownModel"]

let estimate_tokens_fn : js_function =
  [%mel.raw {| compatibility_surface.estimateTokens |}]
[@@mel.as "estimateTokens"]

let estimate_prompt_tokens_fn : js_function =
  [%mel.raw {| compatibility_surface.estimatePromptTokens |}]
[@@mel.as "estimatePromptTokens"]

let estimate_tokens_many_fn : js_function =
  [%mel.raw {| compatibility_surface.estimateTokensMany |}]
[@@mel.as "estimateTokensMany"]

let estimate_tokens_with_ratio_fn : js_function =
  [%mel.raw {| compatibility_surface.estimateTokensWithRatio |}]
[@@mel.as "estimateTokensWithRatio"]

let estimate_message_tokens_fn : js_function =
  [%mel.raw {| compatibility_surface.estimateMessageTokens |}]
[@@mel.as "estimateMessageTokens"]

let estimate_chat_tokens_fn : js_function =
  [%mel.raw {| compatibility_surface.estimateChatTokens |}]
[@@mel.as "estimateChatTokens"]

let estimate_system_prompt_tokens_fn : js_function =
  [%mel.raw {| compatibility_surface.estimateSystemPromptTokens |}]
[@@mel.as "estimateSystemPromptTokens"]

let get_message_overhead_fn : js_function =
  [%mel.raw {| compatibility_surface.getMessageOverhead |}]
[@@mel.as "getMessageOverhead"]

let calculate_available_tokens_fn : js_function =
  [%mel.raw {| compatibility_surface.calculateAvailableTokens |}]
[@@mel.as "calculateAvailableTokens"]

let messages_fit_budget_fn : js_function =
  [%mel.raw {| compatibility_surface.messagesFitBudget |}]
[@@mel.as "messagesFitBudget"]

let calculate_cost_fn : js_function =
  [%mel.raw {| compatibility_surface.calculateCost |}]
[@@mel.as "calculateCost"]

let calculate_cost_custom_fn : js_function =
  [%mel.raw {| compatibility_surface.calculateCostCustom |}]
[@@mel.as "calculateCostCustom"]

let calculate_batch_cost_fn : js_function =
  [%mel.raw {| compatibility_surface.calculateBatchCost |}]
[@@mel.as "calculateBatchCost"]

let calculate_image_cost_fn : js_function =
  [%mel.raw {| compatibility_surface.calculateImageCost |}]
[@@mel.as "calculateImageCost"]

let calculate_video_cost_fn : js_function =
  [%mel.raw {| compatibility_surface.calculateVideoCost |}]
[@@mel.as "calculateVideoCost"]

let estimate_cost_fn : js_function =
  [%mel.raw {| compatibility_surface.estimateCost |}]
[@@mel.as "estimateCost"]

let get_cost_per_token_fn : js_function =
  [%mel.raw {| compatibility_surface.getCostPerToken |}]
[@@mel.as "getCostPerToken"]

let get_pricing_fn : js_function =
  [%mel.raw {| compatibility_surface.getPricing |}]
[@@mel.as "getPricing"]

let has_pricing_fn : js_function =
  [%mel.raw {| compatibility_surface.hasPricing |}]
[@@mel.as "hasPricing"]

let get_context_window_fn : js_function =
  [%mel.raw {| compatibility_surface.getContextWindow |}]
[@@mel.as "getContextWindow"]

let has_context_window_fn : js_function =
  [%mel.raw {| compatibility_surface.hasContextWindow |}]
[@@mel.as "hasContextWindow"]

let split_text_fn : js_function =
  [%mel.raw {| compatibility_surface.splitText |}]
[@@mel.as "splitText"]

let truncate_content_fn : js_function =
  [%mel.raw {| compatibility_surface.truncateContent |}]
[@@mel.as "truncateContent"]

let fit_messages_fn : js_function =
  [%mel.raw {| compatibility_surface.fitMessages |}]
[@@mel.as "fitMessages"]

let paginate_messages_fn : js_function =
  [%mel.raw {| compatibility_surface.paginateMessages |}]
[@@mel.as "paginateMessages"]

let estimate_chunk_count_fn : js_function =
  [%mel.raw {| compatibility_surface.estimateChunkCount |}]
[@@mel.as "estimateChunkCount"]

let provider_base_url_fn : js_function =
  [%mel.raw {| compatibility_surface.providerBaseUrl |}]
[@@mel.as "providerBaseUrl"]

let provider_api_key_fn : js_function =
  [%mel.raw {| compatibility_surface.providerApiKey |}]
[@@mel.as "providerApiKey"]

let provider_request_raw_fn : js_function =
  [%mel.raw {| compatibility_surface.providerRequestRaw |}]
[@@mel.as "providerRequestRaw"]

let provider_request_fn : js_function =
  [%mel.raw {| compatibility_surface.providerRequest |}]
[@@mel.as "providerRequest"]

let provider_request_get_fn : js_function =
  [%mel.raw {| compatibility_surface.providerRequestGet |}]
[@@mel.as "providerRequestGet"]

let provider_list_models_fn : js_function =
  [%mel.raw {| compatibility_surface.providerListModels |}]
[@@mel.as "providerListModels"]

let provider_list_model_ids_fn : js_function =
  [%mel.raw {| compatibility_surface.providerListModelIds |}]
[@@mel.as "providerListModelIds"]

let provider_get_model_fn : js_function =
  [%mel.raw {| compatibility_surface.providerGetModel |}]
[@@mel.as "providerGetModel"]

let provider_model_exists_fn : js_function =
  [%mel.raw {| compatibility_surface.providerModelExists |}]
[@@mel.as "providerModelExists"]

let openai_compatible_chat_fn : js_function =
  [%mel.raw {| compatibility_surface.openAICompatibleChat |}]
[@@mel.as "openAICompatibleChat"]

let openai_compatible_chat_simple_fn : js_function =
  [%mel.raw {| compatibility_surface.openAICompatibleChatSimple |}]
[@@mel.as "openAICompatibleChatSimple"]

let openai_compatible_chat_structured_fn : js_function =
  [%mel.raw {| compatibility_surface.openAICompatibleChatStructured |}]
[@@mel.as "openAICompatibleChatStructured"]

let openai_compatible_stream_fn : js_function =
  [%mel.raw {| compatibility_surface.openAICompatibleStream |}]
[@@mel.as "openAICompatibleStream"]

let openai_compatible_stream_accumulate_fn : js_function =
  [%mel.raw {| compatibility_surface.openAICompatibleStreamAccumulate |}]
[@@mel.as "openAICompatibleStreamAccumulate"]

let stream_readable_from_async_iterable_fn : js_function =
  [%mel.raw {| compatibility_surface.streamReadableFromAsyncIterable |}]
[@@mel.as "streamReadableFromAsyncIterable"]

let provider_embed_fn : js_function =
  [%mel.raw {| compatibility_surface.providerEmbed |}]
[@@mel.as "providerEmbed"]

let provider_embed_one_fn : js_function =
  [%mel.raw {| compatibility_surface.providerEmbedOne |}]
[@@mel.as "providerEmbedOne"]

let provider_embed_many_fn : js_function =
  [%mel.raw {| compatibility_surface.providerEmbedMany |}]
[@@mel.as "providerEmbedMany"]

let cosine_similarity_fn : js_function =
  [%mel.raw {| compatibility_surface.cosineSimilarity |}]
[@@mel.as "cosineSimilarity"]

let find_similar_fn : js_function =
  [%mel.raw {| compatibility_surface.findSimilar |}]
[@@mel.as "findSimilar"]

let provider_generate_image_fn : js_function =
  [%mel.raw {| compatibility_surface.providerGenerateImage |}]
[@@mel.as "providerGenerateImage"]

let provider_generate_image_url_fn : js_function =
  [%mel.raw {| compatibility_surface.providerGenerateImageUrl |}]
[@@mel.as "providerGenerateImageUrl"]

let provider_generate_image_base64_fn : js_function =
  [%mel.raw {| compatibility_surface.providerGenerateImageBase64 |}]
[@@mel.as "providerGenerateImageBase64"]

let provider_generate_images_fn : js_function =
  [%mel.raw {| compatibility_surface.providerGenerateImages |}]
[@@mel.as "providerGenerateImages"]

let provider_start_video_generation_fn : js_function =
  [%mel.raw {| compatibility_surface.providerStartVideoGeneration |}]
[@@mel.as "providerStartVideoGeneration"]

let provider_get_video_status_fn : js_function =
  [%mel.raw {| compatibility_surface.providerGetVideoStatus |}]
[@@mel.as "providerGetVideoStatus"]

let provider_generate_video_fn : js_function =
  [%mel.raw {| compatibility_surface.providerGenerateVideo |}]
[@@mel.as "providerGenerateVideo"]

let provider_generate_video_url_fn : js_function =
  [%mel.raw {| compatibility_surface.providerGenerateVideoUrl |}]
[@@mel.as "providerGenerateVideoUrl"]

let make_openai_strict_fn : js_function =
  [%mel.raw {| compatibility_surface.makeOpenAIStrict |}]
[@@mel.as "makeOpenAIStrict"]

let needs_openai_strict_transform_fn : js_function =
  [%mel.raw {| compatibility_surface.needsOpenAIStrictTransform |}]
[@@mel.as "needsOpenAIStrictTransform"]
...
let xai_client : js_function = [%mel.raw {| makeProviderClientClass("xai") |}]
[@@mel.as "XAIClient"]
...
let create_openai_client : js_function =
  [%mel.raw
    {| function createOpenAIClient(config = {}) { return createProviderClient("openai", config); } |}]
[@@mel.as "createOpenAIClient"]

let create_anthropic_client : js_function =
  [%mel.raw
    {| function createAnthropicClient(config = {}) { return createProviderClient("anthropic", config); } |}]
[@@mel.as "createAnthropicClient"]

let create_xai_client : js_function =
  [%mel.raw
    {| function createXAIClient(config = {}) { return createProviderClient("xai", config); } |}]
[@@mel.as "createXAIClient"]

let create_local_client : js_function =
  [%mel.raw
    {| function createLocalClient(config = {}) { return createProviderClient("local", config); } |}]
[@@mel.as "createLocalClient"]

let create_openrouter_client : js_function =
  [%mel.raw
    {| function createOpenRouterClient(config = {}) { return createProviderClient("openrouter", config); } |}]
...
Object.freeze({
  ...
  // Literal on purpose: the module-level DEFAULT_TIMEOUT const is emitted after
  // this block runs, so referencing it here freezes `undefined` into the namespace.
...
Object.freeze({
  ...
  listModelIds: (options = {}) => providerListModelIds("xai", options),
...
Object.freeze({
  ...
  getModel: (modelId, options = {}) => providerGetModel("openrouter", modelId, options),
  modelExists: (modelId, options = {}) => providerModelExists("openrouter", modelId, options),
...
let tokens_namespace : js_value = [%mel.raw {| compatibility_surface.Tokens |}]
[@@mel.as "Tokens"]
...
let public_gen_stream_accumulate : js_function =
  [%mel.raw {| genStreamAccumulate |}]
...
let public_create_provider_client : js_function =
  [%mel.raw {| createProviderClient |}]

let public_create_openai_client : js_function =
  [%mel.raw {| createOpenAIClient |}]

let public_create_anthropic_client : js_function =
  [%mel.raw {| createAnthropicClient |}]

let public_create_xai_client : js_function = [%mel.raw {| createXAIClient |}]

let public_create_local_client : js_function =
  [%mel.raw {| createLocalClient |}]

let public_create_openrouter_client : js_function =
  [%mel.raw {| createOpenRouterClient |}]
...
let public_detect_provider_by_model : js_function =
  [%mel.raw {| detectProviderByModel |}]
...
let public_assert_provider_active : js_function =
  [%mel.raw {| assertProviderActive |}]
...
let public_get_models_for_provider : js_function =
  [%mel.raw {| getModelsForProvider |}]

let public_get_all_known_models : js_function =
  [%mel.raw {| getAllKnownModels |}]
...
let public_estimate_prompt_tokens : js_function =
  [%mel.raw {| estimatePromptTokens |}]

let public_estimate_tokens_many : js_function =
  [%mel.raw {| estimateTokensMany |}]

let public_estimate_tokens_with_ratio : js_function =
  [%mel.raw {| estimateTokensWithRatio |}]

let public_estimate_message_tokens : js_function =
  [%mel.raw {| estimateMessageTokens |}]

let public_estimate_chat_tokens : js_function =
  [%mel.raw {| estimateChatTokens |}]

let public_estimate_system_prompt_tokens : js_function =
  [%mel.raw {| estimateSystemPromptTokens |}]

let public_get_message_overhead : js_function =
  [%mel.raw {| getMessageOverhead |}]

let public_calculate_available_tokens : js_function =
  [%mel.raw {| calculateAvailableTokens |}]

let public_messages_fit_budget : js_function =
  [%mel.raw {| messagesFitBudget |}]

let public_calculate_cost : js_function = [%mel.raw {| calculateCost |}]

let public_calculate_cost_custom : js_function =
  [%mel.raw {| calculateCostCustom |}]

let public_calculate_batch_cost : js_function =
  [%mel.raw {| calculateBatchCost |}]

let public_calculate_image_cost : js_function =
  [%mel.raw {| calculateImageCost |}]

let public_calculate_video_cost : js_function =
  [%mel.raw {| calculateVideoCost |}]
...
let public_estimate_chunk_count : js_function =
  [%mel.raw {| estimateChunkCount |}]
...
let chat_session_json () =
  ...
  | Error _ -> "{}"
  | Ok _ ->
      Js_session.to_json session |> Chatoyant_js_base.Runtime.Json.to_string
...
let chat_stream_accumulate_json () =
  ...
  in
  let session =
    Js_session.create ~model:"gpt-4o" () |> Js_session.user "Hello"
  in
...
let anthropic_decode_text response_json =
  ...
  | Error message -> "ERROR:" ^ message
  | Ok json ->
      json |> Chatoyant_js_base.Provider.Anthropic.response_of_json
      |> Chatoyant_js_base.Provider.Anthropic.text_of_response

let anthropic_stream_text chunks =
  match
    Chatoyant_js_base.Provider.Anthropic.response_of_stream_chunks chunks
  with
  | Error message -> "ERROR:" ^ message
  | Ok response ->
    ...
    Chatoyant_js_base.Provider.Anthropic.
      {
        ...
        system_blocks = [];
        messages =
          [ { message_role = User; message_content = [ Text "Hello" ] } ];
        max_tokens = 32;
        stream = false;
        temperature = Some 0.0;
        top_p = None;
...
let anthropic_model_count models_json =
  ...
  | Ok json ->
      List.length
        (Chatoyant_js_base.Provider.Anthropic.model_list_of_json json).models

let anthropic_batch_result_text jsonl =
  match
    Chatoyant_js_base.Provider.Anthropic.batch_result_lines_of_jsonl jsonl
  with
  | Error message -> "ERROR:" ^ message
  ...
  let schema =
    Chatoyant_js_base.Runtime.Json.Object
      [
        ...
        ( "required",
          Chatoyant_js_base.Runtime.Json.Array
            [ Chatoyant_js_base.Runtime.Json.String "answer" ] );
...
let openai_decode_text response_json =
  ...
  | Error message -> "ERROR:" ^ message
  | Ok json ->
      (Chatoyant_js_base.Provider.Openai.chat_response_of_json json)
...
let openai_responses_decode_text response_json =
  ...
  | Error message -> "ERROR:" ^ message
  | Ok json ->
      (Chatoyant_js_base.Provider.Openai.responses_response_of_json json)
        .responses_output_text

let openai_stream_text chunks =
  match
    Chatoyant_js_base.Provider.Openai.chat_response_of_stream_chunks chunks
  with
  | Error message -> "ERROR:" ^ message
...
let local_stream chunks =
  match
    Chatoyant_js_base.Provider.Local.chat_response_of_stream_chunks chunks
  with
  | Error message -> ("ERROR:" ^ message, "")
  | Ok response ->
      (response.chat_response_content, response.chat_response_reasoning_content)
...
let xai_decode_text response_json =
  ...
  | Error message -> "ERROR:" ^ message
  | Ok json ->
      (Chatoyant_js_base.Provider.Xai.chat_response_of_json json)
        .response_content
...
let xai_responses_decode_text response_json =
  ...
  | Error message -> "ERROR:" ^ message
  | Ok json ->
      (Chatoyant_js_base.Provider.Xai.responses_response_of_json json)
        .responses_output_text
