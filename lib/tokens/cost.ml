...
type source = Provider_reported | Estimated | Unmetered | Unknown
...
let pricing ?cached_per_million ?cache_write_per_million
    ?cache_write_1h_per_million ?per_image ?per_second ~input_per_million
    ~output_per_million () =
  {
    input_per_million;
    output_per_million;
  ...
  | _ -> (
      ...
      | Some pricing ->
          ...
          in
          {
            ...
            cache_write;
            actual_usd = None;
            total = input +. output +. cached +. cache_write;
          })
...
let calculate_video ~pricing ~duration_seconds =
  match pricing with
  | Some { per_second = Some price; _ } ->
      price *. Float.of_int duration_seconds
  | _ -> 0.0
...
let cost_per_token pricing =
  {
    ...
    per_token_cached =
      Option.value pricing.cached_per_million ~default:0.0 /. 1_000_000.0;
