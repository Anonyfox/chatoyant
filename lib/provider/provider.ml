type id = Openai | Anthropic | Xai | Local | Openrouter
...
let error_to_string = function
  | Missing_api_key { provider; env_key } ->
      Printf.sprintf "missing %s API key from %s" (string_of_id provider)
        env_key
...
let tool_definition_to_json (tool : tool_definition) =
  [ ("name", string tool.tool_name); ("parameters", tool.tool_parameters) ]
  ...
  [
    ...
    ( "tool_calls",
      Chatoyant_runtime.Json.Array
        (List.map tool_call_to_json generation.tool_calls) );
  ]
