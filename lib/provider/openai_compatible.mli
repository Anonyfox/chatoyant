...
val normalize_chat_request :
  config -> Openai.chat_request -> Openai.chat_request

val chat_request_json :
  config -> Openai.chat_request -> Chatoyant_runtime.Json.t

val responses_request_json :
  config -> responses_request -> Chatoyant_runtime.Json.t

val image_request_json : config -> image_request -> Chatoyant_runtime.Json.t

val embedding_request_json :
  config -> embedding_request -> Chatoyant_runtime.Json.t

val chat_response_of_json : config -> Chatoyant_runtime.Json.t -> chat_response

val responses_response_of_json :
  config -> Chatoyant_runtime.Json.t -> responses_response

val generation_of_chat_response : chat_response -> Provider.generation

val chat_response_of_stream_chunks :
  config -> string list -> (chat_response, string) result

val response_of_stream_chunks :
  config -> string list -> (responses_response, string) result

module Make_client (Http : Chatoyant_runtime.Effect.HTTP) : sig
  val create_chat :
    config -> Openai.chat_request -> (chat_response, Openai.api_error) result
