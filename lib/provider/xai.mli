...
(** Chat Completions request body for xAI's [/v1/chat/completions] endpoint.
    Optional fields intentionally stay explicit instead of being inherited from
    OpenAI, because xAI evolves capabilities and unsupported parameters on its
    own schedule. [chat_extra] is the forward-compatible escape hatch for
    freshly shipped provider fields. *)
...
type responses_stream_event =
  ...
  | Response_raw_event of {
      ...
      (** xAI Responses SSE events. The provider currently follows OpenAI's
          event names for [/v1/responses], while this type remains owned by the
          xAI module so downstream explicit-provider code never depends on
          OpenAI internals. *)
...
val collection_search_request_json :
  collection_search_request -> Chatoyant_runtime.Json.t
...
val video_start_response_of_json :
  Chatoyant_runtime.Json.t -> video_start_response

val video_status_response_of_json :
  Chatoyant_runtime.Json.t -> video_status_response
...
val collection_document_of_json :
  Chatoyant_runtime.Json.t -> collection_document
...
module Make_client (Http : Chatoyant_runtime.Effect.HTTP) : sig
  ...
  val synthesize_speech :
    config -> tts_request -> (audio_body, api_error) result

  val list_tts_voices : config -> (voice_list, api_error) result

  val transcribe_speech :
    config -> stt_request -> (stt_response, api_error) result
  ...
  val create_custom_voice :
    config -> custom_voice_request -> (voice, api_error) result

  val list_custom_voices :
    ?limit:int ->
    ?pagination_token:string ->
    config ->
    (voice_list, api_error) result

  val retrieve_custom_voice :
    config -> voice_id:string -> (voice, api_error) result

  val update_custom_voice :
    config ->
    voice_id:string ->
    custom_voice_update ->
    (voice, api_error) result

  val download_custom_voice_audio :
    config -> voice_id:string -> (audio_body, api_error) result

  val delete_custom_voice :
    config -> voice_id:string -> (voice_delete, api_error) result
  ...
  val add_batch_requests :
    ...
    batch_id:string ->
    batch_requests_add ->
    (batch_request_metadata_list, api_error) result
  ...
  val start_video :
    config -> video_request -> (video_start_response, api_error) result

  val get_video_status :
    config -> request_id:string -> (video_status_response, api_error) result
