  ...
  [
    ...
    ( "documents",
      Chatoyant_runtime.Json.Array
        (List.map rerank_document_json request.rerank_documents) );
  ]
...
let video_request_json request =
  ...
  |> add_non_empty "frame_images"
       (fun value -> value)
       request.video_frame_images
  |> add_opt "generate_audio" bool request.video_generate_audio
  |> add_non_empty "input_references"
       (fun value -> value)
       request.video_input_references
...
let model_endpoint_of_json json =
  {
    ...
    model_endpoint_max_completion_tokens =
      int_field "max_completion_tokens" json;
    model_endpoint_quantization = string_field "quantization" json;
    model_endpoint_status = string_field "status" json;
    model_endpoint_supported_parameters =
      string_list_field "supported_parameters" json;
...
let rerank_response_of_json json =
  {
    ...
    rerank_results =
      ...
      | Some (Chatoyant_runtime.Json.Array values) ->
          List.map rerank_result_of_json values
      | _ -> []);
...
let video_model_list_of_json json =
  {
    video_models =
      (match field "data" json with
      | Some (Chatoyant_runtime.Json.Array values) ->
          List.map video_model_of_json values
      | _ -> []);
...
let management_list_of_json json =
  ...
  in
  {
    management_data = List.map management_resource_of_json values;
    management_raw = json;
  }
...
let compatible_config api_key timeout_ms http_referer title headers =
  Openai_compatible.openrouter_config ?timeout_ms ?http_referer ?title ~headers
    ~api_key ()
...
module Make_client (Http : Chatoyant_runtime.Effect.HTTP) = struct
  ...
  let to_compatible config =
    compatible_config config.api_key config.timeout_ms config.http_referer
      config.title config.headers
  ...
  let model_endpoints_path ~author ~slug =
    "/models/"
    ^ pct_encode_path_segment author
    ^ "/"
    ^ pct_encode_path_segment slug
    ^ "/endpoints"
    ...
    Error
      {
        ...
        error_message =
          "OpenRouter model_id must be in author/slug form for endpoint \
           metadata: " ^ model_id;
        error_code = None;
  ...
  let list_providers config =
    send provider_list_of_json
      (request ~method_:"GET" config "/providers" Empty)
  ...
  let rerank config request_body =
    send rerank_response_of_json
      (request config "/rerank" (Json (rerank_request_json request_body)))

  let create_video config request_body =
    send video_job_of_json
      (request config "/videos" (Json (video_request_json request_body)))

  let get_video config ~job_id =
    send video_job_of_json
      (request ~method_:"GET" config ("/videos/" ^ job_id) Empty)
  ...
  let list_video_models config =
    send video_model_list_of_json
      (request ~method_:"GET" config "/videos/models" Empty)
  ...
  let management_post config ~path body =
    send management_resource_of_json
      (management_request ~method_:"POST" config path (Json body))

  let management_patch config ~path body =
    send management_resource_of_json
      (management_request ~method_:"PATCH" config path (Json body))

  let management_delete config ~path =
    send management_delete_of_json
      (management_request ~method_:"DELETE" config path Empty)

  let list_keys config = management_list config ~path:"/keys"
  let get_current_key config = management_get config ~path:"/key"
  let create_key config body = management_post config ~path:"/keys" body
  ...
  let list_guardrails config = management_list config ~path:"/guardrails"
