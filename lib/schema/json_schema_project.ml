...
type result = { schema : Ast.t; warnings : warning list }
...
let replace_field name value fields =
  List.map
    (fun (field_name, field_value) ->
      if field_name = name then (field_name, value)
...
let required_names fields =
  ...
  | Some (Json.Array values) ->
      List.filter_map
        (function Json.String name -> Some name | _ -> None)
        values
  | _ -> []
...
let add_required_properties fields =
  ...
  if properties = [] then fields
    ...
    in
    let required =
      Json.Array (List.map (fun name -> Json.String name) merged)
    in
...
let rec project_json path json =
  ...
  | Json.Object fields ->
      ...
      let fields =
        if
          has_field "properties" fields
          && not (has_field "additionalProperties" fields)
  ...
  | Json.Array values ->
      Json.Array
        (List.mapi
           (fun index -> project_json (path ^ "/" ^ string_of_int index))
           values)
  | value -> value
  ...
  | "dependentSchemas" -> (
      ...
      | Json.Object fields ->
          Json.Object
            (List.map
               (fun (name, schema) ->
                 (name, project_json (path ^ "/" ^ name) schema))
               fields)
      | _ -> value)
