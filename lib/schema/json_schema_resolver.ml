...
type target = { uri : string; base_uri : string; schema : Json.t }
type resource = { uri : string; schema : Json.t }
...
let normalize_path path =
  ...
  let trailing_slash =
    String.length path > 1 && path.[String.length path - 1] = '/'
  in
  let parts =
  ...
  if reference = "" then split_document base_uri
    ...
    if String.length reference > 0 && reference.[0] = '#' then
      ...
      | Some (scheme, authority, _) ->
          scheme ^ "://" ^ authority ^ normalize_path reference
...
let json_string_field name = function
  | Json.Object fields ->
      Option.bind (List.assoc_opt name fields) Json.as_string
  | _ -> None
...
let array_values = function Json.Array values -> values | _ -> []
  ...
  let rec loop base schema (resources, anchors, dynamic_anchors) =
    ...
    | Json.Object fields ->
        ...
        let dynamic_anchors =
          ...
          | Some anchor ->
              add_unique (base ^ "#" ^ anchor) schema dynamic_anchors
        in
        List.fold_left
          (fun acc (name, value) ->
            if is_schema_keyword name then
              ...
              name = "$defs" || name = "definitions" || name = "properties"
              || name = "patternProperties" || name = "dependentSchemas"
            then
              List.fold_left
                (fun acc value -> loop base value acc)
                acc (object_values value)
            else if is_schema_array_keyword name then
              List.fold_left
                (fun acc value -> loop base value acc)
                acc (array_values value)
            else acc)
...
let scan_resource ~retrieval_uri schema (resources, anchors, dynamic_anchors) =
  ...
  in
  let scanned_resources, scanned_anchors, scanned_dynamic_anchors =
    scan base_uri schema
  in
  ...
  let resources =
    add_unique (without_fragment retrieval_uri) schema resources
  in
...
let child_base_uri _registry ~base_uri schema =
  ...
  | Some id ->
      ...
      if
        base_uri <> ""
        && (base_uri = resolved
           || (not (has_scheme id_document))
              && string_ends_with base_uri id_document)
      then base_uri
      else resolved
...
let resolve registry ~base_uri reference =
  ...
  let document = if document = "" then split_document base_uri else document in
  ...
  | Pointer.Pointer tokens -> (
      ...
      | Some schema -> (
          ...
          | Some schema ->
              Some
                {
                  uri = document ^ "#" ^ Pointer.pointer_to_string tokens;
                  base_uri = document;
                  schema;
                }))
  | Pointer.Anchor anchor -> (
      ...
      | Some schema ->
          Some { uri = document ^ "#" ^ anchor; base_uri = document; schema }
      | None -> (
          match
            List.assoc_opt (document ^ "#" ^ anchor) registry.dynamic_anchors
          with
          | Some schema ->
              Some
                { uri = document ^ "#" ^ anchor; base_uri = document; schema }
          | None -> None))
