(** Deterministic provider-oriented schema projections.
    ...
    These helpers derive provider-safe artifacts from a master JSON Schema. They
    never mutate the caller's schema in place. Unsupported constructs are
    preserved unless a projection needs to tighten an object for a known
    provider contract. *)

type warning = { path : string; message : string }
type result = { schema : Json_schema_ast.t; warnings : warning list }
