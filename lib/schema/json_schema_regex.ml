...
type quantifier = Once | Star | Plus | At_least of int
...
let atom_matches atom ch =
  ...
  | Char_class ranges ->
      List.exists (fun (lo, hi) -> ch >= lo && ch <= hi) ranges
...
let parse_tokens pattern =
  ...
  let last =
    if anchored_end then String.length pattern - 1 else String.length pattern
  in
