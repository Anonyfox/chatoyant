...
type tool_call = { id : string; name : string; arguments_json : string }
...
let make ?name ?tool_call_id ?tool_result_error ?(tool_calls = [])
...
let tool ?is_error ~tool_call_id content =
  make ?tool_result_error:is_error ~tool_call_id Tool content
...
let metadata_to_json metadata = Chatoyant_runtime.Json.Object metadata
...
let to_json message =
  [
    ...
    ( "tool_calls",
      Chatoyant_runtime.Json.Array
        (List.map tool_call_to_json message.tool_calls) );
  ...
  |> add_opt "tool_result_error"
       (Option.map
          (fun value -> Chatoyant_runtime.Json.Bool value)
          message.tool_result_error)
  |> List.rev
...
let tool_call_of_json json =
  match
    ( string_field "id" json,
      ...
      string_field "arguments_json" json )
  with
          ...
          Ok
            {
              ...
              tool_result_error =
                Option.bind
                  (field "tool_result_error" json)
                  Chatoyant_runtime.Json.as_bool;
              tool_calls;
