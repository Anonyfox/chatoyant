...
| Messages | `src/core/message.ts` | `Chatoyant_core.Message` | scaffolded |
...
| Provider detection | `src/providers/detection.ts` | `Chatoyant_provider.Registry` | scaffolded |
| OpenAI raw client | `src/providers/openai/*` | `Chatoyant_provider.Openai`, `Openai_decode`, `Openai_stream` | standalone Responses/chat/images/audio/embeddings/models/files/vector stores/batches/moderation/fine-tuning client, stream decode, typed errors, tests |
...
| xAI raw client | `src/providers/xai/*` | `Chatoyant_provider.Xai` | standalone Responses/chat/image/edit/video/models/files/collections/batches/REST voice/custom voice client, stream decode, typed errors, tests |
...
- `docs/PROVIDER_FEATURE_MATRIX.md` is the current-docs audit target for raw
  provider parity. Each gap should be closed against official docs before being
  marked `done`.
...
- OpenAI Realtime WebRTC/WebSocket support after runtime transport boundaries
  exist.
...
- xAI mTLS and WebSocket-mode fixtures once those runtime transport boundaries
  land. REST deferred request retrieval, Responses SSE, REST TTS/STT, and custom
  voice endpoints are covered.
- Local live smoke when a local OpenAI-compatible server is available through
  `LOCAL_BASE_URL`.
