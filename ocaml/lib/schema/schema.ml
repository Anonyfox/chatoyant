...
let string ?description ?optional ?default ?min_length ?max_length ?pattern ?format () =
  String
    {
...
let boolean ?description ?optional ?default () =
  Boolean { boolean_base = base ?description ?optional (); boolean_default = default }
...
let enum ?description ?optional values =
  Enum { enum_base = base ?description ?optional (); values; enum_default = None }
...
let with_array_constraints ?min_items ?max_items ?unique_items = function
  | Array options ->
      let keep previous next = match next with Some _ -> next | None -> previous in
      Array
        {
          options with
...
let add_base base fields =
  add_opt "description" (fun value -> Chatoyant_runtime.Json.String value) base.description fields
...
let number_fields options kind =
  ...
  |> add_opt "default" (fun value -> Chatoyant_runtime.Json.Float value) options.number_default
  |> add_opt "minimum" (fun value -> Chatoyant_runtime.Json.Float value) options.minimum
  |> add_opt "maximum" (fun value -> Chatoyant_runtime.Json.Float value) options.maximum
  |> add_opt "exclusiveMinimum" (fun value -> Chatoyant_runtime.Json.Float value) options.exclusive_minimum
  |> add_opt "exclusiveMaximum" (fun value -> Chatoyant_runtime.Json.Float value) options.exclusive_maximum
  |> add_opt "multipleOf" (fun value -> Chatoyant_runtime.Json.Float value) options.multiple_of
...
let rec to_property = function
  | String options ->
      ...
      |> add_opt "default" (fun value -> Chatoyant_runtime.Json.String value) options.string_default
      |> add_opt "minLength" (fun value -> Chatoyant_runtime.Json.Float (Float.of_int value)) options.min_length
      |> add_opt "maxLength" (fun value -> Chatoyant_runtime.Json.Float (Float.of_int value)) options.max_length
      |> add_opt "pattern" (fun value -> Chatoyant_runtime.Json.String value) options.pattern
      |> add_opt "format" (fun value -> Chatoyant_runtime.Json.String (string_of_format value)) options.format
      |> List.rev |> fun fields -> Chatoyant_runtime.Json.Object fields
  | Number options -> Chatoyant_runtime.Json.Object (List.rev (number_fields options "number"))
  | Integer options -> Chatoyant_runtime.Json.Object (List.rev (number_fields options "integer"))
  | Boolean options ->
      [
        ("type", Chatoyant_runtime.Json.String "boolean");
      ]
      |> add_base options.boolean_base
      |> add_opt "default" (fun value -> Chatoyant_runtime.Json.Bool value) options.boolean_default
      |> List.rev |> fun fields -> Chatoyant_runtime.Json.Object fields
  | Null base -> Chatoyant_runtime.Json.Object (List.rev (add_base base [ ("type", Chatoyant_runtime.Json.String "null") ]))
  | Array options ->
      [
      ...
      |> add_opt "minItems" (fun value -> Chatoyant_runtime.Json.Float (Float.of_int value)) options.min_items
      |> add_opt "maxItems" (fun value -> Chatoyant_runtime.Json.Float (Float.of_int value)) options.max_items
      |> add_opt "uniqueItems" (fun value -> Chatoyant_runtime.Json.Bool value) options.unique_items
      |> List.rev |> fun fields -> Chatoyant_runtime.Json.Object fields
  | Object options ->
      let properties =
        options.fields
      ...
      let required =
        ...
        |> List.filter_map (fun (name, field) ->
               if is_optional field then None else Some (Chatoyant_runtime.Json.String name))
...
let to_json_schema field =
  ...
  | Chatoyant_runtime.Json.Object fields ->
      Chatoyant_runtime.Json.Object (("$schema", Chatoyant_runtime.Json.String "https://json-schema.org/draft/2020-12/schema") :: fields)
  | value -> value
