...
type options = {
  ...
  resources : Resolver.resource list;
}
...
let validation_vocabulary = "https://json-schema.org/draft/2020-12/vocab/validation"
...
let merge results =
  {
    ...
    evaluated_properties =
      results |> List.concat_map (fun result -> result.evaluated_properties) |> uniq;
    evaluated_items =
...
let vocabulary_validation_enabled ctx ~base_uri schema =
  ...
  | Some schema_uri -> (
      ...
      | None -> ctx.validation_enabled
      | Some target -> (
          match Option.bind (json_field "$vocabulary" target.schema) json_object with
          | None -> true
          | Some vocabularies -> (
              match Option.bind (List.assoc_opt validation_vocabulary vocabularies) json_bool with
              | Some true -> true
...
let utf8_length value =
  ...
  String.iter
    (fun ch ->
      if Char.code ch land 0xC0 <> 0x80 then incr count)
    value;
  !count
...
let schema_append base keyword =
  pointer_append base keyword
...
let valid_type_name = function
  | "null" | "boolean" | "object" | "array" | "number" | "string" | "integer" -> true
  | _ -> false

let non_negative_integer_keyword = function
  | "minLength" | "maxLength" | "minItems" | "maxItems" | "minProperties"
  | "maxProperties" | "minContains" | "maxContains" -> true
  | _ -> false
...
let rec validate_schema_document ~instance_path ~schema_path schema =
  ...
  | Json.Object fields ->
      ...
      |> List.map (fun (name, value) ->
             ...
             if name = "type" then validate_type_keyword_shape ~instance_path:path ~schema_path value
             else if non_negative_integer_keyword name then
               validate_non_negative_integer_shape ~instance_path:path ~schema_path ~keyword:name value
...
and validate_type_keyword_shape ~instance_path ~schema_path = function
  ...
  | Json.Array values
    when values <> []
         && List.for_all
              (function Json.String value -> valid_type_name value | _ -> false)
              values ->
      empty
  | _ ->
...
and validate_non_negative_integer_shape ~instance_path ~schema_path ~keyword = function
...
and validate_schema_array_shape ~instance_path ~schema_path = function
  | Json.Array values ->
      ...
      |> List.mapi (fun index value ->
             validate_schema_document ~instance_path:(index_path instance_path index)
               ~schema_path value)
      |> merge
  | _ ->
...
and validate_schema_map_shape ~instance_path ~schema_path = function
  | Json.Object fields ->
      fields
      |> List.map (fun (name, value) ->
             validate_schema_document ~instance_path:(pointer_append instance_path name)
               ~schema_path value)
      |> merge
  | _ ->
...
let rec validate_schema ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
    instance =
  match schema with
  | Json.Bool true -> empty
  | Json.Bool false -> fail ~instance_path ~schema_path ~keyword:"false" "boolean schema false"
  | Json.Object _ ->
      validate_object_schema ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
        instance
  | _ ->
      fail ~instance_path ~schema_path ~keyword:"schema"
        "schema position must contain a boolean or object"

and validate_object_schema ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
    instance =
  ...
  let ref_results =
    [
      validate_ref_keyword ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
        instance;
      validate_dynamic_ref_keyword ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope
        schema instance;
    ]
  ...
  let assertion_results =
    (if ctx.validation_enabled then
       [
         ...
         validate_string ~schema_path ~instance_path ctx.options schema instance;
       ]
     else [])
    @
    [
      validate_object_keywords ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
        instance;
      validate_array_keywords ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
        instance;
      validate_dependent_keywords ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope
        schema instance;
    ]
    |> List.filter_map Fun.id
  in
  let applicator_results =
    validate_applicators ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema instance
  in
  let before_unevaluated = merge (ref_results @ assertion_results @ applicator_results) in
  let unevaluated_results =
    [
      validate_unevaluated_properties ctx ~base_uri ~schema_path ~instance_path
        ~dynamic_scope ~evaluated:before_unevaluated.evaluated_properties schema instance;
      validate_unevaluated_items ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope
        ~evaluated:before_unevaluated.evaluated_items schema instance;
    ]
    |> List.filter_map Fun.id
  in
  finish (merge (ref_results @ assertion_results @ applicator_results @ unevaluated_results))
  with exn ->
    ctx.validation_enabled <- previous_validation_enabled;
    raise exn

and validate_ref_keyword ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
    instance =
  ...
  | Some reference -> (
      ...
      | None ->
          Some
            (fail ~instance_path ~schema_path:(schema_append schema_path "$ref")
               ~keyword:"$ref"
               ("unresolved reference " ^ reference))
      | Some target ->
          if target.uri = draft202012_meta_schema then
            Some (validate_schema_document ~instance_path ~schema_path:target.uri instance)
          else
            ...
            if List.mem stack_key ctx.ref_stack then Some empty
              ...
              let result =
                validate_schema ctx ~base_uri:target.base_uri ~schema_path:target.uri
                  ~instance_path ~dynamic_scope target.schema instance
              in
              ctx.ref_stack <- List.tl ctx.ref_stack;
              Some result))

and validate_dynamic_ref_keyword ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope
    schema instance =
  match Option.bind (json_field "$dynamicRef" schema) json_string with
  | None -> None
  | Some reference -> (
      match Resolver.resolve_dynamic ctx.resolver ~base_uri ~dynamic_scope reference with
      | None ->
          Some
            (fail ~instance_path ~schema_path:(schema_append schema_path "$dynamicRef")
               ~keyword:"$dynamicRef"
...
and validate_type ~schema_path ~instance_path schema instance =
  ...
  | Some (Json.String expected) ->
      if type_matches expected instance then Some empty
        ...
        Some
          (fail ~instance_path ~schema_path:(schema_append schema_path "type")
             ~keyword:"type"
             ("expected " ^ expected ^ ", got " ^ kind instance))
  | Some (Json.Array values) ->
      let expected = List.filter_map json_string values in
      if List.exists (fun expected -> type_matches expected instance) expected then Some empty
      else
        Some
          (fail ~instance_path ~schema_path:(schema_append schema_path "type")
             ~keyword:"type"
             ("expected one of " ^ String.concat ", " expected ^ ", got " ^ kind instance))
  | Some _ -> None
...
and validate_enum ~schema_path ~instance_path schema instance =
  ...
  | Some values ->
      if List.exists (json_equal instance) values then Some empty
        ...
        Some
          (fail ~instance_path ~schema_path:(schema_append schema_path "enum")
...
and validate_const ~schema_path ~instance_path schema instance =
  ...
  | Some value ->
      if json_equal instance value then Some empty
        ...
        Some
          (fail ~instance_path ~schema_path:(schema_append schema_path "const")
  ...
  | Json.Float number ->
      let checks =
        [
          ...
          ("maximum", (fun maximum -> number <= maximum), "number is greater than maximum");
          ("exclusiveMaximum", (fun maximum -> number < maximum), "number is not below exclusiveMaximum");
          ("minimum", (fun minimum -> number >= minimum), "number is less than minimum");
          ("exclusiveMinimum", (fun minimum -> number > minimum), "number is not above exclusiveMinimum");
        ]
      in
      let results =
        List.filter_map
          (fun (keyword, predicate, message) ->
            ...
            | Some bound ->
                if predicate bound then Some empty
                  ...
                  Some
                    (fail ~instance_path ~schema_path:(schema_append schema_path keyword)
                       ~keyword message))
          checks
      in
      Some (merge results)
  ...
  | Json.String text ->
      ...
      let min_result =
        ...
        | Some minimum when length < minimum ->
            fail ~instance_path ~schema_path:(schema_append schema_path "minLength")
      ...
      let max_result =
        ...
        | Some maximum when length > maximum ->
            fail ~instance_path ~schema_path:(schema_append schema_path "maxLength")
      ...
      let pattern_result =
        ...
        | Some pattern when not (Regex.matches ~pattern text) ->
            fail ~instance_path ~schema_path:(schema_append schema_path "pattern")
              ~keyword:"pattern" "string does not match pattern"
        | _ -> empty
      in
      let format_result =
        if options.format_assertion then validate_format ~schema_path ~instance_path schema text
        else empty
      in
...
and validate_format ~schema_path ~instance_path schema text =
  ...
  | Some "email" ->
      if String.contains text '@' then empty
        ...
        fail ~instance_path ~schema_path:(schema_append schema_path "format")
...
and validate_object_keywords ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
    instance =
  match instance with
  | Json.Object fields ->
      let count = List.length fields in
      let min_result =
        match (ctx.validation_enabled, Option.bind (json_field "minProperties" schema) json_int) with
        | true, Some minimum when count < minimum ->
            fail ~instance_path ~schema_path:(schema_append schema_path "minProperties")
              ~keyword:"minProperties" "object has too few properties"
        | _ -> empty
      in
      let max_result =
        match (ctx.validation_enabled, Option.bind (json_field "maxProperties" schema) json_int) with
        | true, Some maximum when count > maximum ->
            fail ~instance_path ~schema_path:(schema_append schema_path "maxProperties")
              ~keyword:"maxProperties" "object has too many properties"
        | _ -> empty
      in
      let required_result =
        match (ctx.validation_enabled, Option.bind (json_field "required" schema) json_array) with
        | false, _ -> empty
        | true, None -> empty
        | true, Some required ->
            required
            ...
            |> List.map (fun name ->
                   if List.mem_assoc name fields then empty
                     ...
                     fail ~instance_path ~schema_path:(schema_append schema_path "required")
                       ~keyword:"required" ("missing required property " ^ name))
            |> merge
      in
      let property_result, property_names =
        validate_properties ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
          fields
      in
      let pattern_result, pattern_names =
        validate_pattern_properties ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope
          schema fields
      in
      let additional_result =
        validate_additional_properties ctx ~base_uri ~schema_path ~instance_path
          ~dynamic_scope schema fields ~covered:(uniq (property_names @ pattern_names))
      in
      let property_names_result =
        validate_property_names ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
          fields
      in
      Some
        (merge
...
and validate_properties ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema fields =
            ...
            | Some value ->
                let result =
                  validate_schema ctx ~base_uri
                    ~schema_path:(schema_append (schema_append schema_path "properties") name)
...
and validate_pattern_properties ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope
    schema fields =
              ...
              (fun (results, names) (name, value) ->
                if Regex.matches ~pattern name then
                  let result =
                    validate_schema ctx ~base_uri
                      ~schema_path:
                        (schema_append (schema_append schema_path "patternProperties") pattern)
                      ~instance_path:(pointer_append instance_path name)
                      ~dynamic_scope subschema value
                    |> as_child_property name
                  in
...
and validate_additional_properties ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope
...
and validate_property_names ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
    fields =
...
and validate_dependent_keywords ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope
    schema instance =
  match instance with
  | Json.Object fields ->
      let dependent_required =
        match (ctx.validation_enabled, Option.bind (json_field "dependentRequired" schema) json_object) with
        | false, _ -> empty
        | true, None -> empty
      ...
      let dependent_schemas =
        match Option.bind (json_field "dependentSchemas" schema) json_object with
        | None -> empty
        | Some dependencies ->
            dependencies
            |> List.filter_map (fun (name, subschema) ->
                   if List.mem_assoc name fields then
                     Some
                       (validate_schema ctx ~base_uri
                          ~schema_path:
                            (schema_append (schema_append schema_path "dependentSchemas") name)
...
and validate_array_keywords ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
    instance =
  match instance with
  | Json.Array values ->
      let length = List.length values in
      let min_result =
        match (ctx.validation_enabled, Option.bind (json_field "minItems" schema) json_int) with
        | true, Some minimum when length < minimum ->
            fail ~instance_path ~schema_path:(schema_append schema_path "minItems")
              ~keyword:"minItems" "array has too few items"
        | _ -> empty
      in
      let max_result =
        match (ctx.validation_enabled, Option.bind (json_field "maxItems" schema) json_int) with
        | true, Some maximum when length > maximum ->
            fail ~instance_path ~schema_path:(schema_append schema_path "maxItems")
              ~keyword:"maxItems" "array has too many items"
        | _ -> empty
      in
      let unique_result =
        match (ctx.validation_enabled, Option.bind (json_field "uniqueItems" schema) json_bool) with
        | true, Some true when not (unique_items values) ->
            fail ~instance_path ~schema_path:(schema_append schema_path "uniqueItems")
              ~keyword:"uniqueItems" "array items are not unique"
        | _ -> empty
      in
      let prefix_result, prefix_count =
        validate_prefix_items ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
          values
      in
      let items_result =
        validate_items ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema values
          ~prefix_count
      in
      let contains_result =
        validate_contains ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
          values
      in
      Some
        (merge
...
and unique_items values =
  let rec loop = function
    | [] -> true
    | value :: rest ->
        (not (List.exists (json_equal value) rest)) && loop rest
  in
  loop values

and validate_prefix_items ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema
    values =
...
and validate_items ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema values
    ~prefix_count =
  ...
  | Some subschema ->
      ...
      |> List.map (fun (index, value) ->
             validate_schema ctx ~base_uri ~schema_path:(schema_append schema_path "items")
               ~instance_path:(index_path instance_path index)
               ~dynamic_scope subschema value
             |> as_child_item index)
      |> merge

and validate_contains ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema values =
  ...
  | Some subschema ->
      ...
      let min_result =
        if count < min_contains then
          fail ~instance_path ~schema_path:(schema_append schema_path "contains")
            ~keyword:"contains" "array does not contain enough matching items"
        else empty
      in
      let max_result =
        match (ctx.validation_enabled, Option.bind (json_field "maxContains" schema) json_int) with
        | true, Some maximum when count > maximum ->
            fail ~instance_path ~schema_path:(schema_append schema_path "maxContains")
      ...
      if result.valid then
        {
          result with
          evaluated_items = List.map fst matches;
        }
      else result

and validate_applicators ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope schema instance =
  let all_of =
    ...
    | Some schemas ->
        ...
        |> List.mapi (fun index subschema ->
               validate_schema ctx ~base_uri
                 ~schema_path:(index_path (schema_append schema_path "allOf") index)
                 ~instance_path ~dynamic_scope subschema instance)
        |> merge
  in
  let any_of =
    ...
    | Some schemas ->
        let results =
          ...
          |> List.mapi (fun index subschema ->
                 validate_schema ctx ~base_uri
                   ~schema_path:(index_path (schema_append schema_path "anyOf") index)
                   ~instance_path ~dynamic_scope subschema instance)
        in
        let passing = List.filter (fun result -> result.valid) results in
        if passing = [] then
          fail ~instance_path ~schema_path:(schema_append schema_path "anyOf")
  ...
  let one_of =
    ...
    | Some schemas ->
        let results =
          ...
          |> List.mapi (fun index subschema ->
                 validate_schema ctx ~base_uri
                   ~schema_path:(index_path (schema_append schema_path "oneOf") index)
                   ~instance_path ~dynamic_scope subschema instance)
        in
        let passing = List.filter (fun result -> result.valid) results in
        if List.length passing = 1 then merge passing
        else
          fail ~instance_path ~schema_path:(schema_append schema_path "oneOf")
  ...
  let not_ =
    ...
    | Some subschema ->
        let result =
          validate_schema ctx ~base_uri ~schema_path:(schema_append schema_path "not")
            ~instance_path ~dynamic_scope subschema instance
        in
        if result.valid then
          fail ~instance_path ~schema_path:(schema_append schema_path "not") ~keyword:"not"
            "value matched forbidden subschema"
        else empty
  in
  let conditional =
    ...
    | Some if_schema ->
        let if_result =
          validate_schema ctx ~base_uri ~schema_path:(schema_append schema_path "if")
        ...
        if if_result.valid then
          ...
          | None -> empty
          | Some else_schema ->
              validate_schema ctx ~base_uri ~schema_path:(schema_append schema_path "else")
                ~instance_path ~dynamic_scope else_schema instance
  in
  [ all_of; any_of; one_of; not_; conditional ]

and validate_unevaluated_properties ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope
    ~evaluated schema instance =
  ...
  | Some subschema, Json.Object fields ->
      ...
      |> List.map (fun (name, value) ->
             validate_schema ctx ~base_uri
               ...
               ~dynamic_scope subschema value
             |> as_child_property name)
      |> merge |> fun result -> Some result
  | Some _, _ -> Some empty

and validate_unevaluated_items ctx ~base_uri ~schema_path ~instance_path ~dynamic_scope
    ~evaluated schema instance =
...
let validate ?(options = default_options) schema instance =
  ...
  let result =
    validate_schema ctx ~base_uri:(Resolver.root_base_uri resolver) ~schema_path:""
      ~instance_path:"" ~dynamic_scope:[] schema_json instance
  in
