...
let json ?description () = { schema = Schema.object_ ?description []; encode = Fun.id; decode = ok; missing = None }
...
let string_enum ?description values =
  let json_values = List.map (fun value -> Chatoyant_runtime.Json.String value) values in
  {
    schema = Schema.enum ?description json_values;
    encode = (fun value -> Chatoyant_runtime.Json.String value);
    decode =
      (function
      | Chatoyant_runtime.Json.String value when List.mem value values -> Ok value
      | Chatoyant_runtime.Json.String value -> Error ("unexpected enum value: " ^ value)
...
let with_optional = function
  | Schema.String options ->
      Schema.String { options with string_base = set_base_optional true options.string_base }
  | Schema.Number options ->
      Schema.Number { options with number_base = set_base_optional true options.number_base }
  | Schema.Integer options ->
      Schema.Integer { options with number_base = set_base_optional true options.number_base }
  | Schema.Boolean options ->
      Schema.Boolean { options with boolean_base = set_base_optional true options.boolean_base }
  | Schema.Null base -> Schema.Null (set_base_optional true base)
  | Schema.Array options ->
      Schema.Array { options with array_base = set_base_optional true options.array_base }
  | Schema.Object options ->
      Schema.Object { options with object_base = set_base_optional true options.object_base }
  | Schema.Enum options ->
      Schema.Enum { options with enum_base = set_base_optional true options.enum_base }
  | Schema.Literal options ->
      Schema.Literal { options with literal_base = set_base_optional true options.literal_base }
...
let encode_field record field = (field.name, field.codec.encode (field.get record))
...
let object1 ?description f1 make =
  {
    ...
    encode = (fun record -> Chatoyant_runtime.Json.Object [ encode_field record f1 ]);
...
let object2 ?description f1 f2 make =
  {
    ...
    encode =
      (fun record ->
        Chatoyant_runtime.Json.Object [ encode_field record f1; encode_field record f2 ]);
    decode =
      (fun json ->
...
let object3 ?description f1 f2 f3 make =
  {
    schema =
      Schema.object_ ?description [ schema_field f1; schema_field f2; schema_field f3 ];
    encode =
      (fun record ->
        Chatoyant_runtime.Json.Object
          [ encode_field record f1; encode_field record f2; encode_field record f3 ]);
    decode =
      (fun json ->
        let ( let* ) = Result.bind in
        let* v1 = decode_record_field json f1 in
...
let object5 ?description f1 f2 f3 f4 f5 make =
  {
    schema =
      Schema.object_ ?description
        [ schema_field f1; schema_field f2; schema_field f3; schema_field f4; schema_field f5 ];
    encode =
      (fun record ->
        Chatoyant_runtime.Json.Object
          [
