let p ?cached ?cache_write_5m ?cache_write_1h ?per_image ?per_second ~input ~output () =
  Cost.pricing ?cached_per_million:cached ?cache_write_per_million:cache_write_5m
    ?cache_write_1h_per_million:cache_write_1h ?per_image ?per_second ~input_per_million:input
...
let all =
  [
    ...
    ("claude-opus-4-6", p ~input:5.0 ~output:25.0 ~cached:0.5 ~cache_write_5m:6.25 ~cache_write_1h:10.0 ());
    ("claude-sonnet-4-6", p ~input:3.0 ~output:15.0 ~cached:0.3 ~cache_write_5m:3.75 ~cache_write_1h:6.0 ());
    ("claude-opus-4-5-20251101", p ~input:5.0 ~output:25.0 ~cached:0.5 ~cache_write_5m:6.25 ~cache_write_1h:10.0 ());
    ("claude-opus-4-5", p ~input:5.0 ~output:25.0 ~cached:0.5 ~cache_write_5m:6.25 ~cache_write_1h:10.0 ());
    ("claude-sonnet-4-5-20250929", p ~input:3.0 ~output:15.0 ~cached:0.3 ~cache_write_5m:3.75 ~cache_write_1h:6.0 ());
    ("claude-sonnet-4-5", p ~input:3.0 ~output:15.0 ~cached:0.3 ~cache_write_5m:3.75 ~cache_write_1h:6.0 ());
    ("claude-haiku-4-5-20251001", p ~input:1.0 ~output:5.0 ~cached:0.1 ~cache_write_5m:1.25 ~cache_write_1h:2.0 ());
    ("claude-haiku-4-5", p ~input:1.0 ~output:5.0 ~cached:0.1 ~cache_write_5m:1.25 ~cache_write_1h:2.0 ());
    ("claude-opus-4-1-20250805", p ~input:15.0 ~output:75.0 ~cached:1.5 ~cache_write_5m:18.75 ~cache_write_1h:30.0 ());
    ("claude-opus-4-1", p ~input:15.0 ~output:75.0 ~cached:1.5 ~cache_write_5m:18.75 ~cache_write_1h:30.0 ());
    ("claude-sonnet-4-20250514", p ~input:3.0 ~output:15.0 ~cached:0.3 ~cache_write_5m:3.75 ~cache_write_1h:6.0 ());
    ("claude-sonnet-4-0", p ~input:3.0 ~output:15.0 ~cached:0.3 ~cache_write_5m:3.75 ~cache_write_1h:6.0 ());
    ("claude-opus-4-20250514", p ~input:15.0 ~output:75.0 ~cached:1.5 ~cache_write_5m:18.75 ~cache_write_1h:30.0 ());
    ("claude-opus-4-0", p ~input:15.0 ~output:75.0 ~cached:1.5 ~cache_write_5m:18.75 ~cache_write_1h:30.0 ());
    ("claude-3-5-sonnet-20241022", p ~input:3.0 ~output:15.0 ~cached:0.3 ~cache_write_5m:3.75 ~cache_write_1h:6.0 ());
    ("claude-3-5-sonnet-20240620", p ~input:3.0 ~output:15.0 ~cached:0.3 ~cache_write_5m:3.75 ~cache_write_1h:6.0 ());
    ("claude-3-5-haiku-20241022", p ~input:0.8 ~output:4.0 ~cached:0.08 ~cache_write_5m:1.0 ~cache_write_1h:1.6 ());
    ("claude-3-opus-20240229", p ~input:15.0 ~output:75.0 ~cached:1.5 ~cache_write_5m:18.75 ~cache_write_1h:30.0 ());
    ("claude-3-haiku-20240307", p ~input:0.25 ~output:1.25 ~cached:0.03 ~cache_write_5m:0.3 ~cache_write_1h:0.5 ());
    ("grok-4.20-0309-reasoning", p ~input:2.0 ~output:6.0 ~cached:0.2 ());
    ("grok-4.20-0309-non-reasoning", p ~input:2.0 ~output:6.0 ~cached:0.2 ());
    ("grok-4.20-multi-agent-0309", p ~input:2.0 ~output:6.0 ~cached:0.2 ());
...
let family_model model =
  ...
  if starts_with ~prefix:"claude" lower && contains lower "opus" then Some "claude-opus-4-6"
  else if starts_with ~prefix:"claude" lower && contains lower "sonnet" then
    Some "claude-sonnet-4-6"
  else if starts_with ~prefix:"claude" lower && contains lower "haiku" then Some "claude-haiku-4-5"
  else if starts_with ~prefix:"gpt-" lower && contains lower "-nano" then Some "gpt-5.4-nano"
  else if starts_with ~prefix:"gpt-" lower && contains lower "-mini" then Some "gpt-5.4-mini"
  else if starts_with ~prefix:"gpt-" lower && contains lower "-pro" then Some "gpt-5.4-pro"
  else if starts_with ~prefix:"gpt-" lower && contains lower "-codex" then Some "gpt-5.2-codex"
  else if starts_with ~prefix:"gpt-" lower && contains lower "o" then Some "gpt-4o"
  else if starts_with ~prefix:"gpt-" lower then Some "gpt-5.4"
  else if starts_with ~prefix:"o" lower && contains lower "-mini" then Some "o4-mini"
  else if starts_with ~prefix:"o" lower && contains lower "-pro" then Some "o3-pro"
  ...
  else if starts_with ~prefix:"grok-" lower && contains lower "fast-reasoning" then
    Some "grok-4-1-fast-reasoning"
  else if starts_with ~prefix:"grok-" lower && contains lower "fast-non-reasoning" then
    Some "grok-4-1-fast-non-reasoning"
  else if starts_with ~prefix:"grok-" lower && contains lower "mini" then Some "grok-3-mini"
  else if starts_with ~prefix:"grok-" lower && contains lower "code" then Some "grok-code-fast-1"
  else if starts_with ~prefix:"grok-imagine-image-pro" lower then Some "grok-imagine-image-pro"
  else if starts_with ~prefix:"grok-imagine-image" lower then Some "grok-imagine-image"
  else if starts_with ~prefix:"grok-imagine-video" lower then Some "grok-imagine-video"
  else if starts_with ~prefix:"grok-" lower then Some "grok-4.20-0309-reasoning"
...
let provider_max_model model =
  ...
  else if starts_with ~prefix:"gpt-" lower || starts_with ~prefix:"chatgpt" lower || starts_with ~prefix:"o" lower
  then Some "o1-pro"
...
let get ?fallback model =
  ...
  | None -> (
      match Option.bind (family_model model) (fun model -> List.assoc_opt model all) with
      | Some _ as pricing -> pricing
      | None -> (
          match Option.bind (provider_max_model model) (fun model -> List.assoc_opt model all) with
          | Some _ as pricing -> pricing
          | None -> fallback))
