...
let segments ?separator text =
  ...
  | Some separator ->
      split_on_substring ~separator text |> List.map trim |> List.filter (( <> ) "")
...
let split_large_segment ~max_tokens ~overlap segment =
  let rec loop current current_tokens chunks = function
    | [] ->
        if trim current = "" then List.rev chunks else List.rev (trim current :: chunks)
    | word :: rest ->
    ...
    let rec loop current current_tokens chunks = function
      ...
      | segment :: rest ->
          ...
          if segment_tokens > max_tokens then
            let flushed = if trim current = "" then chunks else trim current :: chunks in
            let large_chunks = split_large_segment ~max_tokens ~overlap segment in
            loop "" 0 (List.rev_append large_chunks flushed) rest
          else
            let added_tokens = current_tokens + segment_tokens + if current = "" then 0 else 1 in
            if current <> "" && added_tokens > max_tokens then
              let overlap_text = overlap_text current overlap in
              let next = if overlap_text = "" then segment else overlap_text ^ " " ^ segment in
              loop next (Token_estimate.estimate next) (trim current :: chunks) rest
            else
  ...
  if content = "" then content
    ...
    if tokens <= max_tokens then content
      ...
      if target_chars <= 0 then ellipsis
        ...
        let truncated = String.sub content 0 (min target_chars (String.length content)) in
        let last_space =
          try Some (String.rindex truncated ' ') with
          | Not_found -> None
        in
        let truncated =
          match last_space with
          | Some index when Float.of_int index > Float.of_int target_chars *. 0.8 ->
              String.sub truncated 0 index
          | _ -> truncated
        in
        trim truncated ^ ellipsis

let fit_messages ?(provider = Message_budget.Openai) ~max_tokens ?(reserve_for_response = 0)
    messages =
  Message_budget.fit ~provider ~max_tokens ~reserve_for_response messages

let paginate_messages ?(provider = Message_budget.Openai) ~tokens_per_page messages =
  let rec loop current current_tokens pages = function
    | [] ->
        if current = [] then List.rev pages else List.rev (List.rev current :: pages)
    | message :: rest ->
        let message_tokens = Message_budget.estimate_message ~provider message in
        if current <> [] && current_tokens + message_tokens > tokens_per_page then
          loop [ message ] message_tokens (List.rev current :: pages) rest
        else loop (message :: current) (current_tokens + message_tokens) pages rest
  in
  loop [] 0 [] messages

let estimate_chunk_count ~chunk_size text =
  if text = "" || chunk_size <= 0 then 0
  else int_of_float (ceil (Float.of_int (Token_estimate.estimate text) /. Float.of_int chunk_size))
