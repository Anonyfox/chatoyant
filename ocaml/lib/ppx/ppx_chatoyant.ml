...
let some ~loc expr =
  B.pexp_construct ~loc (lident ~loc "Some") (Some expr)
      ...
      [
        {
          pstr_desc =
            Pstr_eval
              ( {
                  pexp_desc = Pexp_constant (Pconst_float (value, _));
                  _;
                },
                _ );
          _;
          ...
          pstr_desc =
            Pstr_eval
              ( {
                  pexp_desc =
                    Pexp_construct
                      ({ txt = Longident.Lident ("true" | "false" as value); _ }, None);
                  _;
                },
                _ );
          _;
...
let merge_metadata first second =
  {
    description =
      (match first.description with Some _ -> first.description | None -> second.description);
    min_items = (match first.min_items with Some _ -> first.min_items | None -> second.min_items);
    max_items = (match first.max_items with Some _ -> first.max_items | None -> second.max_items);
    unique_items =
      (match first.unique_items with
      | Some _ -> first.unique_items
      | None -> second.unique_items);
    min_length =
      (match first.min_length with Some _ -> first.min_length | None -> second.min_length);
    max_length =
      (match first.max_length with Some _ -> first.max_length | None -> second.max_length);
    pattern = (match first.pattern with Some _ -> first.pattern | None -> second.pattern);
    minimum = (match first.minimum with Some _ -> first.minimum | None -> second.minimum);
    maximum = (match first.maximum with Some _ -> first.maximum | None -> second.maximum);
  }
...
let pexp_float ~loc value =
  B.pexp_constant ~loc (Pconst_float (value, None))

let call ~loc name args =
  B.pexp_apply ~loc (evar ~loc name) args
...
let apply_metadata ~loc metadata codec =
  if not (has_metadata metadata) then codec
    ...
    let schema =
      let args =
        [
          Option.map (fun value -> (Labelled "min_items", eint ~loc value)) metadata.min_items;
          Option.map (fun value -> (Labelled "max_items", eint ~loc value)) metadata.max_items;
          Option.map
      ...
      in
      if args = [] then schema
      else call ~loc "Chatoyant.Schema.Schema.with_array_constraints" (args @ [ (Nolabel, schema) ])
    in
    let schema =
      let args =
        [
          Option.map (fun value -> (Labelled "min_length", eint ~loc value)) metadata.min_length;
          Option.map (fun value -> (Labelled "max_length", eint ~loc value)) metadata.max_length;
          Option.map (fun value -> (Labelled "pattern", estring ~loc value)) metadata.pattern;
        ]
        |> List.filter_map Fun.id
      in
      if args = [] then schema
      else call ~loc "Chatoyant.Schema.Schema.with_string_constraints" (args @ [ (Nolabel, schema) ])
    in
    let schema =
      let args =
        [
      ...
      in
      if args = [] then schema
      else call ~loc "Chatoyant.Schema.Schema.with_number_constraints" (args @ [ (Nolabel, schema) ])
    in
    [%expr
      Chatoyant.Schema.Codec.map_schema
        (fun schema -> [%e schema])
        [%e codec]]
...
let tool_name_from_module name =
  ...
  if String.length raw > 0 && raw.[0] = '_' then String.sub raw 1 (String.length raw - 1)
  else raw
...
let rec append_codec_suffix = function
  ...
  | Longident.Lapply _ ->
      Location.raise_errorf "chatoyant.ppx cannot derive a codec path for this type"
...
let rec codec_expr_of_type ~type_names typ =
  ...
  let codec =
    ...
    | Ptyp_constr
        ...
        [%expr Chatoyant.Schema.Codec.array [%e codec_expr_of_type ~type_names item]]
    | Ptyp_constr ({ txt = Longident.Lident "option"; _ }, [ item ])
    | Ptyp_constr
        ({ txt = Longident.Ldot (Longident.Lident "Option", "t"); _ }, [ item ]) ->
        [%expr Chatoyant.Schema.Codec.optional [%e codec_expr_of_type ~type_names item]]
    ...
    | Ptyp_constr ({ txt = Longident.Ldot _ as name; _ }, []) ->
        B.pexp_apply ~loc
          (B.pexp_ident ~loc
             (B.Located.mk ~loc (append_codec_suffix name)))
          [ (Nolabel, eunit ~loc) ]
    | _ ->
        raise_errorf ~loc
...
let schema_field_expr ~type_names field =
  ...
  tuple2 ~loc (estring ~loc field.pld_name.txt)
    [%expr Chatoyant.Schema.Codec.schema [%e field_codec_expr ~type_names field]]
...
let encode_field_expr ~type_names field =
  ...
  tuple2 ~loc (estring ~loc field.pld_name.txt)
    [%expr
      Chatoyant.Schema.Codec.encode [%e field_codec_expr ~type_names field]
...
let decode_field_binding ~type_names field body =
  ...
  let rhs =
    [%expr
      Chatoyant.Schema.Codec.decode_field [%e estring ~loc field.pld_name.txt]
        [%e field_codec_expr ~type_names field] json]
  in
  bind_result ~loc rhs field.pld_name.txt body

let local_type ~loc type_name =
  B.ptyp_constr ~loc (lident ~loc type_name) []
...
let record_codec_body ~type_names ~type_name ?description fields =
  ...
  let schema =
    let args =
      ...
      @ [ (Nolabel, B.elist ~loc (List.map (schema_field_expr ~type_names) fields)) ]
    in
    call ~loc "Chatoyant.Schema.Schema.object_" args
  in
  [%expr
    Chatoyant.Schema.Codec.custom
      ~schema:[%e schema]
      ~encode:
        (fun (value : [%t local_type ~loc type_name.txt]) ->
  ...
  let encode_cases =
    List.map
      (fun (constructor, json_name) ->
        B.case ~lhs:(B.ppat_construct ~loc (lident ~loc constructor.pcd_name.txt) None)
          ~guard:None
    ...
    List.map
      (fun (constructor, json_name) ->
        B.case
          ...
          ~rhs:[%expr Ok [%e B.pexp_construct ~loc (lident ~loc constructor.pcd_name.txt) None]])
      cases
  in
  let decode_cases =
    decode_cases
    @ [
        B.case
          ...
          ~rhs:
            [%expr
              Error
                ("unexpected " ^ [%e estring ~loc type_name.txt] ^ " value: " ^ value)];
        B.case ~lhs:[%pat? _] ~guard:None
          ~rhs:[%expr Error ("expected " ^ [%e estring ~loc type_name.txt] ^ " string")];
      ]
  in
  let schema =
    let values =
...
let alias_codec_body ~type_names ~metadata typ =
  codec_expr_of_type ~type_names typ |> apply_metadata ~loc:typ.ptyp_loc metadata
...
let codec_body_of_decl ~type_names decl =
  ...
  | Ptype_record fields, None ->
      record_codec_body ~type_names ~type_name:decl.ptype_name ?description fields
  | Ptype_variant constructors, None ->
      variant_codec_body ~type_name:decl.ptype_name ?description constructors
  | Ptype_abstract, Some manifest -> alias_codec_body ~type_names ~metadata manifest
  | _ ->
...
let generated_codec_items ~loc type_decls =
  ...
  let codec_item =
    B.pstr_value ~loc Recursive (List.map (codec_binding ~type_names) type_decls)
  in
  let helper_items =
    type_decls
    |> List.concat_map helper_bindings
    |> fun bindings -> if bindings = [] then [] else [ B.pstr_value ~loc Nonrecursive bindings ]
  in
...
let helper_sig_items decl =
  ...
  [
    ...
    value (type_name ^ "_to_json")
      [%type: [%t typ] -> Chatoyant.Runtime.Json.t];
...
and collect_reachable_decl type_map seen decl =
  ...
  if String_map.mem name seen then seen
    ...
    let seen =
      ...
      | Ptype_record fields ->
          List.fold_left
            (fun seen field -> collect_reachable_type type_map seen field.pld_type)
            seen fields
...
let rec arrows acc typ =
  ...
  | Ptyp_arrow ((Labelled _ | Optional _), _, _) ->
      raise_errorf ~loc:typ.ptyp_loc
        "module%%tool currently expects run to take one request record argument \
         or env -> request"
  | _ -> (List.rev acc, typ)
...
let parse_return_type typ =
  ...
  | Ptyp_constr ({ txt = Longident.Lident "result"; _ }, [ output; error ])
    when is_string_type error ->
      (output, true)
  | Ptyp_constr
      ({ txt = Longident.Ldot (Longident.Lident "Stdlib", "result"); _ }, [ output; error ])
    when is_string_type error ->
      (output, true)
  | _ -> (typ, false)
...
let parse_run_shape binding =
  ...
  match args with
  | [ input_type ] -> { env_type = None; input_type; output_type; returns_result }
  | [ env_type; input_type ] ->
...
let generated_tool_item ~loc ~module_name ~description ~type_names shape =
  ...
  in
  let body_with_env =
    if shape.returns_result then [%expr run env input] else [%expr Ok (run env input)]
  in
      ...
      [%stri
        let tool =
          Chatoyant.Core.Tool.create_typed ~name:[%e name_expr]
            ...
            ~result:[%e output_codec]
            (fun _context input -> [%e body_no_env])]
  | Some _ ->
      [%stri
        let make env =
          Chatoyant.Core.Tool.create_typed ~name:[%e name_expr]
            ~description:[%e description_expr] ~args:[%e input_codec]
            ~result:[%e output_codec]
            (fun _context input -> [%e body_with_env])]
...
let expand_tool_module ~loc binding =
  ...
  | Pmod_structure structure ->
      ...
      in
      {
        binding with
        pmb_expr =
          { binding.pmb_expr with pmod_desc = Pmod_structure (structure @ [ tool_item ]) };
      }
  | _ ->
      raise_errorf ~loc:binding.pmb_loc
...
let expand_gen_data_expression ~loc typ =
  ...
  in
  B.pexp_apply ~loc (evar ~loc "Chatoyant.gen_data")
    [
...
let (_ : Deriving.t) =
  Deriving.add "chatoyant" ~str_type_decl ~sig_type_decl

let () =
  Driver.register_transformation "chatoyant.ppx" ~impl
