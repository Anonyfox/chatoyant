...
module Http = struct
  ...
  let multipart_boundary () =
    "chatoyant-eio-" ^ Int.to_string (Random.bits ()) ^ Int.to_string (Random.bits ())
  ...
  let make ?(https = System) ?(max_response_size = default_max_response_size) ~net ~clock () =
    Mirage_crypto_rng_unix.use_default ();
    let https =
      tls_config_exn https
      |> Option.map (fun config -> https_wrapper config)
    in
    (module struct
      ...
      let encode_multipart parts =
        ...
        let add_part part =
          ...
          let disposition =
            ...
            | None -> "form-data; name=\"" ^ escape_disposition_value part.name ^ "\""
            | Some filename ->
                "form-data; name=\""
      ...
      let prepare_body headers body =
        ...
        | Json json ->
            let headers = add_header_unless_exists "content-type" "application/json" headers in
            (headers, Some (json |> Chatoyant_runtime.Json.to_string |> Cohttp_eio.Body.of_string))
        | Multipart parts ->
      ...
      let send request =
        ...
        | Some timeout_ms when timeout_ms > 0 -> (
            ...
            try Eio.Time.with_timeout_exn clock seconds (fun () -> perform request) with
  ...
  let get_json ?https ?max_response_size ?headers ?timeout_ms ~net ~clock url =
    match get ?https ?max_response_size ?headers ?timeout_ms ~net ~clock url with
    | Error _ as err -> err
  ...
  let post_json ?https ?max_response_size ?(headers = []) ?timeout_ms ~net ~clock url json =
    send ?https ?max_response_size ~net ~clock
...
module Noop_http : Chatoyant_runtime.Effect.HTTP = struct
  ...
  let send _ = Error (Network "noop HTTP module is only used for provider defaults")
end
...
module Provider = struct
  ...
  let openai ?https ?max_response_size ?(base_url = Openai_defaults.default_base_url)
      ?timeout_ms ~net ~clock ~api_key () =
    let module Http = (val http ?https ?max_response_size ~net ~clock ()) in
    let module Provider = Chatoyant_provider.Openai.Make_provider (Http) (struct
      let api_key = api_key
  ...
  let anthropic ?https ?max_response_size
      ?(base_url = Anthropic_defaults.default_base_url)
      ?timeout_ms ?(beta_headers = []) ~net ~clock ~api_key () =
    let module Http = (val http ?https ?max_response_size ~net ~clock ()) in
    let module Provider = Chatoyant_provider.Anthropic.Make_provider (Http) (struct
      let api_key = api_key
      let base_url = base_url
      let timeout_ms = timeout_ms
  ...
  let xai ?https ?max_response_size ?(base_url = Xai_defaults.default_base_url)
      ?timeout_ms ~net ~clock ~api_key () =
    let module Http = (val http ?https ?max_response_size ~net ~clock ()) in
    let module Provider = Chatoyant_provider.Xai.Make_provider (Http) (struct
      let api_key = api_key
      let base_url = base_url
      let timeout_ms = timeout_ms
    end) in
  ...
  let openrouter ?https ?max_response_size ?timeout_ms ?http_referer ?title ?(headers = [])
      ~net ~clock ~api_key () =
    let module Http = (val http ?https ?max_response_size ~net ~clock ()) in
    let module Provider = Chatoyant_provider.Openrouter.Make_provider (Http) (struct
      let api_key = api_key
      let timeout_ms = timeout_ms
  ...
  let local ?https ?max_response_size ?api_key ?timeout_ms ?(headers = []) ~net ~clock ~base_url
      () =
    let module Http = (val http ?https ?max_response_size ~net ~clock ()) in
    let module Provider = Chatoyant_provider.Local.Make_provider (Http) (struct
      let base_url = base_url
      let api_key = api_key
      let timeout_ms = timeout_ms
      let headers = headers
...
module Chat = struct
  module type SESSION = sig
    ...
    val create : ?model:string -> ?defaults:Chatoyant_core.Options.t -> unit -> t
    val model : t -> string
    ...
    let rec chat =
      {
        ...
        set_model_ = (fun model -> ignore (Session.set_model model session); chat);
        messages_ = (fun () -> Session.messages session);
        tools_ = (fun () -> Session.tools session);
        last_result_ = (fun () -> Session.last_result session);
        system_ = (fun content -> ignore (Session.system content session); chat);
        user_ = (fun content -> ignore (Session.user content session); chat);
        assistant_ = (fun content -> ignore (Session.assistant content session); chat);
        add_message_ = (fun message -> ignore (Session.add_message message session); chat);
        add_messages_ = (fun messages -> ignore (Session.add_messages messages session); chat);
        clear_messages_ = (fun () -> ignore (Session.clear_messages session); chat);
        add_tool_ = (fun tool -> ignore (Session.add_tool tool session); chat);
        add_tools_ = (fun tools -> ignore (Session.add_tools tools session); chat);
        clear_tools_ = (fun () -> ignore (Session.clear_tools session); chat);
        generate_with_result_ =
          (fun ?options () -> Session.generate_with_result ?options session);
        generate_ = (fun ?options () -> Session.generate ?options session);
        stream_accumulate_ =
          (fun ?options frames -> Session.stream_accumulate ?options frames session);
        ...
        load_json_ =
          (fun json ->
            ...
            | Error _ as err -> err);
        clone_ =
          (fun () -> pack_session ~provider_id (module Session) (Session.clone session));
        fork_ =
          (fun () -> pack_session ~provider_id (module Session) (Session.fork session));
      }
    in
    chat
  ...
  let with_provider ?model ?defaults ~clock provider =
    ...
    pack_session ~provider_id:Provider.id (module Session)
      (Session.create ?model ?defaults ())

  let openai ?https ?max_response_size ?base_url ?timeout_ms ?model ?defaults env ?api_key
      () =
    let net = env#net in
    ...
    let provider =
      ...
      | Some api_key ->
          Provider.openai ?https ?max_response_size ?base_url ?timeout_ms ~net ~clock
            ~api_key ()
      | None -> missing_api_key_provider Openai "OPENAI_API_KEY"
    in
    with_provider ?model ?defaults ~clock provider

  let anthropic ?https ?max_response_size ?base_url ?timeout_ms ?beta_headers ?model
      ?defaults env ?api_key () =
    let net = env#net in
    let clock = env#clock in
    let provider =
  ...
  let xai ?https ?max_response_size ?base_url ?timeout_ms ?model ?defaults env ?api_key
      () =
    let net = env#net in
    let clock = env#clock in
    let provider =
      ...
      | Some api_key ->
          Provider.xai ?https ?max_response_size ?base_url ?timeout_ms ~net ~clock
            ~api_key ()
  ...
  let openrouter ?https ?max_response_size ?timeout_ms ?http_referer ?title ?headers
    ...
    let provider =
      ...
      | Some api_key ->
          Provider.openrouter ?https ?max_response_size ?timeout_ms ?http_referer
            ?title ?headers ~net ~clock ~api_key ()
      | None -> missing_api_key_provider Openrouter "OPENROUTER_API_KEY"
    in
    with_provider ?model ?defaults ~clock provider

  let local ?https ?max_response_size ?api_key ?timeout_ms ?headers ?model ?defaults env
      ~base_url () =
    let net = env#net in
    let clock = env#clock in
    Provider.local ?https ?max_response_size ?api_key ?timeout_ms ?headers ~net ~clock
      ~base_url ()
  ...
  let options_override ?options ?timeout_ms ?temperature ?max_tokens ?extra () =
    let options = Option.value options ~default:Chatoyant_core.Options.default in
    {
      options with
      timeout_ms = (match timeout_ms with Some _ -> timeout_ms | None -> options.timeout_ms);
      temperature =
        (match temperature with Some _ -> temperature | None -> options.temperature);
      max_tokens =
        (match max_tokens with Some _ -> max_tokens | None -> options.max_tokens);
      extra = (match extra with Some _ -> extra | None -> options.extra);
    }

  let generate_with_result ?options ?timeout_ms ?temperature ?max_tokens ?extra chat =
    let options = options_override ?options ?timeout_ms ?temperature ?max_tokens ?extra () in
  ...
  let prepare_ask ?system:system_prompt ?tools prompt chat =
    ...
    let chat =
      match tools with
      | None -> chat
      | Some tools -> with_tools tools chat
    in
    user prompt chat

  let ask ?system ?tools ?options ?timeout_ms ?temperature ?max_tokens ?extra prompt chat =
    chat
    |> prepare_ask ?system ?tools prompt
    |> generate_with_result ?options ?timeout_ms ?temperature ?max_tokens ?extra

  let ask_text ?system ?tools ?options ?timeout_ms ?temperature ?max_tokens ?extra prompt
      chat =
    chat
    |> prepare_ask ?system ?tools prompt
    |> generate ?options ?timeout_ms ?temperature ?max_tokens ?extra
  let stream_accumulate ?options frames chat = chat.stream_accumulate_ ?options frames
...
let add_default_messages ?system ?tools chat =
  ...
  match tools with
  | None -> chat
  | Some tools -> Chat.with_tools tools chat

let openai ?https ?max_response_size ?base_url ?timeout_ms ?model ?defaults ?api_key
    ?system ?tools env =
  Chat.openai ?https ?max_response_size ?base_url ?timeout_ms ?model ?defaults env
    ?api_key ()
  |> add_default_messages ?system ?tools

let anthropic ?https ?max_response_size ?base_url ?timeout_ms ?beta_headers ?model
    ?defaults ?api_key ?system ?tools env =
  Chat.anthropic ?https ?max_response_size ?base_url ?timeout_ms ?beta_headers
    ?model ?defaults env ?api_key ()
  |> add_default_messages ?system ?tools

let xai ?https ?max_response_size ?base_url ?timeout_ms ?model ?defaults ?api_key
    ?system ?tools env =
...
let local ?https ?max_response_size ?api_key ?timeout_ms ?headers ?model ?defaults
    ?system ?tools env ~base_url =
  Chat.local ?https ?max_response_size ?api_key ?timeout_ms ?headers ?model
    ?defaults env ~base_url ()
  |> add_default_messages ?system ?tools

let gen_result ?system ?tools ?options ?timeout_ms ?temperature ?max_tokens ?extra
    client prompt =
  client
  |> Chat.fork
  |> Chat.ask ?system ?tools ?options ?timeout_ms ?temperature ?max_tokens ?extra
       prompt
...
let structured_extra provider ?name ?description ~strict schema extra =
  ...
  | Chatoyant_provider.Provider.Openai ->
      let fields =
        extra |> object_fields |> remove_json_field "text"
      in
      Some
        (Chatoyant_runtime.Json.Object
  ...
  | Xai | Local | Openrouter ->
      ...
      Some
        (Chatoyant_runtime.Json.Object
           (("response_format", chat_response_format ~name ?description ~strict schema_json)
           :: fields))
  | Anthropic -> extra
...
let gen_data ?name ?description ?(strict = true) ~codec ?system ?tools ?options
  ...
  let extra = structured_extra provider ?name ?description ~strict schema extra in
  let system =
    match provider with
  ...
  | Ok text -> (
      ...
      | Error message -> Error (Chatoyant_provider.Provider.Decode_error message)
      | Ok json -> (
...
module Generate = struct
  let with_provider ?(options = Chatoyant_core.Options.default) ~clock ~provider chat =
    let module Provider = (val provider : Chatoyant_provider.Provider.CHAT) in
    let module Clock = (val Clock.make ~clock) in
