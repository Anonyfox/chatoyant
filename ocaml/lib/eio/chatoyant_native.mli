(** Eio runtime integration for native Chatoyant consumers.
    ...
    This library keeps the provider implementations runtime-neutral while
    making the native path feel natural in Eio applications. Build the HTTP and
    clock effects from an [Eio_main.run] environment, then pass the resulting
    modules to the existing provider/core functors or use the provider
    constructors below. *)
...
module Chat : sig
  ...
  val openai :
    ...
    unit ->
    t
  (** OpenAI chat. When [api_key] is omitted, [OPENAI_API_KEY] is read from
      the process environment and missing keys are reported as Chatoyant
      provider errors on generation. *)
...
(** One-shot typed JSON generation.
    ...
    Prefer [[%chatoyant.gen_data: your_type]] with
    [[@@deriving chatoyant]] so the codec and schema stay invisible at the call
    site. OpenAI uses Responses structured output, OpenAI-compatible providers
    use [response_format], and Anthropic receives a schema instruction plus
    local validation. *)
