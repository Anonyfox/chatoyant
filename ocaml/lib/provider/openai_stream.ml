...
let tool_calls_of_delta delta_json =
  ...
  | Some (Chatoyant_runtime.Json.Array calls) -> List.map tool_call_delta_of_json calls
  | _ -> []
...
let delta_of_json json =
  ...
  | Some choice ->
      ...
      let delta_json = Option.value (field "delta" choice) ~default:Chatoyant_runtime.Json.Null in
      {
