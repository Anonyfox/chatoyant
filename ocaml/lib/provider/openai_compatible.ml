          ...
          if not state.in_thinking then
            ...
            if prefix_of ~target:think_open candidate then
              if candidate = think_open then
                loop (index + 1) { in_thinking = true; pending = "" } content reasoning
              else loop (index + 1) { state with pending = candidate } content reasoning
            else if state.pending <> "" then
              loop (index + 1) { state with pending = "" } (content ^ candidate) reasoning
            else if char_text = "<" then
              loop (index + 1) { state with pending = char_text } content reasoning
            ...
            if prefix_of ~target:think_close candidate then
              if candidate = think_close then
                loop (index + 1) { in_thinking = false; pending = "" } content reasoning
              else loop (index + 1) { state with pending = candidate } content reasoning
            else if state.pending <> "" then
              loop (index + 1) { state with pending = "" } content (reasoning ^ candidate)
            else if char_text = "<" then
              loop (index + 1) { state with pending = char_text } content reasoning
...
let smooth_delta config state delta =
  ...
  | Local ->
      ...
      ({ delta with Openai_stream.delta_content = content; delta_reasoning_content = reasoning }, state)
...
let chat_response_of_stream_chunks config chunks =
  ...
  and decode thinking_state acc events =
    ...
    | [] -> (thinking_state, acc)
    | event :: rest ->
        if Chatoyant_runtime.Sse.is_done event then decode thinking_state acc rest
        else
          ...
          | Ok json ->
              ...
              let delta, thinking_state = smooth_delta config thinking_state delta in
...
module Make_client (Http : Chatoyant_runtime.Effect.HTTP) = struct
          ...
          Error
            {
              ...
              error_message = "OpenAI-compatible HTTP " ^ string_of_int response.status ^ ": " ^ response.body;
              error_code = None;
              error_param = None;
              error_raw = None;
            }
  ...
  let retrieve_model config ~model_id =
    send Openai.model_of_json (request ~method_:"GET" config ("/models/" ^ model_id) Empty)
end
...
struct
  ...
  let generate messages (options : Provider.options) =
    let request =
      {
        ...
        chat_top_logprobs = None;
        chat_n = None;
        chat_tools = List.map openai_function_tool_of_provider_tool options.tools;
    ...
    | Ok response -> Ok (generation_of_chat_response_for_config Config.config response)
