...
val authorization_headers :
  ?http_referer:string -> ?title:string -> api_key:string -> unit -> (string * string) list
...
module Make_client (Http : Chatoyant_runtime.Effect.HTTP) : sig
  ...
  val rerank : config -> rerank_request -> (rerank_response, Openai.api_error) result
  val create_video : config -> video_request -> (video_job, Openai.api_error) result
