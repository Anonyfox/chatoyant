...
let openai_compatible json =
  {
    ...
    reasoning_tokens = nested_int "completion_tokens_details" "reasoning_tokens" json;
    cached_tokens = nested_int "prompt_tokens_details" "cached_tokens" json;
    cache_write_tokens = nested_int "prompt_tokens_details" "cache_write_tokens" json;
...
let anthropic json =
  {
    ...
    total_tokens = int_field "input_tokens" json + int_field "output_tokens" json;
    actual_cost_usd = None;
  }
...
let openrouter json =
  ...
  {
    base with
    actual_cost_usd = if credits > 0.0 then Some (credits *. 0.000001) else None;
  }
