...
let split_complete_frames buffer =
  ...
  let rec scan acc start =
    ...
    | None -> (List.rev acc, String.sub normalized start (String.length normalized - start))
    | Some index ->
...
let parse_line (event, data) line =
  if line = "" || line.[0] = ':' then (event, data)
    ...
    | Some index ->
        ...
        let value_start = if index + 1 < String.length line && line.[index + 1] = ' ' then index + 2 else index + 1 in
