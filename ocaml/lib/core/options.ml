...
let first_non_empty right left =
  match right with
  | [] -> left
  | _ -> right
...
let merge_extra left right =
  ...
  | Some (Chatoyant_runtime.Json.Object left_fields), Some (Chatoyant_runtime.Json.Object right_fields) ->
      let without_overridden =
        List.filter (fun (name, _) -> not (List.mem_assoc name right_fields)) left_fields
      in
...
let merge defaults overrides =
  {
    ...
    local_timeout_ms = first_some overrides.local_timeout_ms defaults.local_timeout_ms;
    ...
    frequency_penalty = first_some overrides.frequency_penalty defaults.frequency_penalty;
    presence_penalty = first_some overrides.presence_penalty defaults.presence_penalty;
    ...
    thinking_budget = first_some overrides.thinking_budget defaults.thinking_budget;
    max_tool_iterations = overrides.max_tool_iterations;
    tool_timeout_ms = first_some overrides.tool_timeout_ms defaults.tool_timeout_ms;
