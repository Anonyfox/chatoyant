...
let timing_to_json timing =
  ...
  |> add_opt "time_to_first_token_ms" (Option.map int timing.time_to_first_token_ms)
...
let token_speed_to_json speed =
  ...
  |> add_opt "output_tokens_per_second" (Option.map float speed.output_tokens_per_second)
  |> add_opt "total_tokens_per_second" (Option.map float speed.total_tokens_per_second)
  |> List.rev |> fun fields -> Chatoyant_runtime.Json.Object fields
...
let generation_to_json generation =
  [
    ...
    ("provider", string (Chatoyant_provider.Provider.string_of_id generation.provider));
    ("model", string generation.model);
    ( "tool_calls",
      Chatoyant_runtime.Json.Array
        (List.map Chatoyant_provider.Provider.tool_call_to_json generation.tool_calls) );
