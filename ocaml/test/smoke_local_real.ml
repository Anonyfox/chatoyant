...
let () =
  ...
  let config =
    Client.{ base_url; api_key = Sys.getenv_opt "LOCAL_API_KEY"; timeout_ms = Some 60_000; headers = [] }
  in
  let request =
    Chatoyant.Provider.Openai.
      {
        ...
        chat_messages =
          [
            {
              message_role = User;
              message_content = Some "Respond with the exact text OCAML_LOCAL_OK and nothing else.";
              message_name = None;
  ...
  | Ok response ->
      if String.trim response.response_content <> "OCAML_LOCAL_OK" then
        failwith ("unexpected local response: " ^ response.response_content);
