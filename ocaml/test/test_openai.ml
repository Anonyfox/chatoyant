...
module Provider = Chatoyant.Provider.Openai.Make_provider (Fake_http) (struct
...
let schema =
  Chatoyant.Runtime.Json.Object
    [
      ...
      ("required", Chatoyant.Runtime.Json.Array [ Chatoyant.Runtime.Json.String "answer" ]);
  ...
  Chatoyant.Provider.Openai.
    {
      ...
      responses_reasoning =
        Some (Chatoyant.Runtime.Json.Object [ ("effort", Chatoyant.Runtime.Json.String "low") ]);
      responses_tools = [ Chatoyant.Runtime.Json.Object [ ("type", Chatoyant.Runtime.Json.String "web_search_preview") ] ];
...
let test_request_json () =
  let chat_body =
    chat_fixture ()
    |> Chatoyant.Provider.Openai.chat_request_json
  ...
  let responses_body =
    responses_fixture ()
    |> Chatoyant.Provider.Openai.responses_request_json
    |> Chatoyant.Runtime.Json.to_string
  in
...
let test_response_decode () =
                     ...
                     ( "message",
                       Chatoyant.Runtime.Json.Object
                         [
                           ...
                           ("reasoning_content", Chatoyant.Runtime.Json.String "Because");
                         ] );
                   ];
               ] );
           ( "usage",
              ...
              ( "choices",
                Chatoyant.Runtime.Json.Array
                  [
                    Chatoyant.Runtime.Json.Object
                      [
                        ("finish_reason", Chatoyant.Runtime.Json.String "tool_calls");
                        ( "message",
                          Chatoyant.Runtime.Json.Object
                            [
                              ("role", Chatoyant.Runtime.Json.String "assistant");
                              ...
                              ( "tool_calls",
                                Chatoyant.Runtime.Json.Array
                                  [
                                    Chatoyant.Runtime.Json.Object
                                      [
                                        ("id", Chatoyant.Runtime.Json.String "call_1");
                                        ("type", Chatoyant.Runtime.Json.String "function");
                                        ( "function",
                                          Chatoyant.Runtime.Json.Object
                                            [
                                              ("name", Chatoyant.Runtime.Json.String "lookup");
                                              ( "arguments",
                                                Chatoyant.Runtime.Json.String "{\"q\":\"ocaml\"}" );
                                            ] );
                                      ];
                                  ] );
                            ] );
  ...
  in
  assert_equal_string "tool_calls" (Option.get tool_call_generation.finish_reason);
  ...
  (match
     Chatoyant.Provider.Openai.chat_response_of_stream_chunks
       [
         ...
         "data: {\"choices\":[{\"finish_reason\":\"stop\",\"delta\":{}}],\"usage\":{\"prompt_tokens\":2,\"completion_tokens\":1,\"total_tokens\":3}}\n\n";
         "data: [DONE]\n\n";
       ]
   with
  | Error message -> failwith message
  ...
  (match
     Chatoyant.Provider.Openai.response_of_stream_chunks
       [
         ...
         "data: {\"type\":\"response.completed\",\"response\":{\"status\":\"completed\",\"output_text\":\"Hello\",\"usage\":{\"input_tokens\":2,\"output_tokens\":1,\"total_tokens\":3}}}\n\n";
       ]
   with
  | Error message -> failwith message
  | Ok response ->
...
let test_other_decoders () =
  ...
  assert_equal_string "abc" (Option.get (List.hd image.image_data).image_b64_json);
  let embedding =
               ...
               [
                 Chatoyant.Runtime.Json.Object
                   [
                     ...
                     ( "embedding",
                       Chatoyant.Runtime.Json.Array
                         [ Chatoyant.Runtime.Json.Float 0.1; Chatoyant.Runtime.Json.Float 0.2 ] );
                   ];
               ] );
           ( "usage",
             Chatoyant.Runtime.Json.Object
  ...
  assert_equal_string "gpt-5.4-mini" (Option.get (List.hd models.models).model_id)
...
let test_client_and_provider () =
  ...
  Fake_http.next_response_body :=
    "{\"error\":{\"type\":\"invalid_request_error\",\"message\":\"bad request\",\"code\":\"bad\",\"param\":\"input\"}}";
  (match Client.create_response config (responses_fixture ()) with
  ...
  | Ok generation -> assert_equal_string "Hello from Responses" generation.content);
...
let test_client_moderation_and_batches () =
  ...
  (match
     Client.create_moderation config
       { moderation_model = None; moderation_input = Moderation_text "hello"; moderation_extra = [] }
   with
  | Error error -> failwith error.error_message
  ...
  | Some request ->
      ...
      | Multipart parts ->
          ...
          if
            not
              (List.exists
                 (fun part -> part.Fake_http.name = "purpose" && part.body = "batch")
                 parts)
    ...
    Chatoyant.Provider.Openai.
      {
        ...
        fine_tuning_hyperparameters =
          Some (Chatoyant.Runtime.Json.Object [ ("n_epochs", Chatoyant.Runtime.Json.Float 2.0) ]);
...
let test_client_media () =
  ...
  (match
   ...
   with
  | Error error -> failwith error.error_message
  | Ok response -> assert_equal_string "abc" (Option.get (List.hd response.image_data).image_b64_json));
  (match !(Fake_http.last_request) with
  | Some request ->
      ...
      if List.exists (fun (name, _) -> String.lowercase_ascii name = "content-type") request.headers then
        failwith "multipart request should let runtime set content-type";
      (match request.body with
      | Multipart parts ->
          if not (List.exists (fun part -> part.Fake_http.name = "image") parts) then
            failwith "missing image part";
          if not (List.exists (fun part -> part.Fake_http.name = "prompt") parts) then
            failwith "missing prompt part"
      | _ -> failwith "expected image edit multipart body")
  | None -> failwith "expected image edit request");
  Fake_http.next_response_body := "{\"text\":\"hello world\",\"language\":\"en\",\"duration\":1.2}";
  (match
   ...
   with
  | Error error -> failwith error.error_message
  | Ok transcript -> assert_equal_string "hello world" transcript.transcription_text);
  (match !(Fake_http.last_request) with
  | Some request -> (
      ...
      | Multipart parts ->
          if
            not
              (List.exists
                 (fun part -> part.Fake_http.name = "timestamp_granularities[]" && part.body = "word")
                 parts)
...
let test_client_vector_and_fine_tuning () =
  ...
  | Ok response ->
      assert_equal_string "file_123"
        (Option.get (List.hd response.vector_store_search_results).vector_store_search_file_id));
  (match !(Fake_http.last_request) with
  ...
  (match
   ...
   with
  | Error error -> failwith error.error_message
  | Ok batch -> assert_equal_string "vsfb_123" (Option.get batch.vector_store_file_batch_id));
  (match !(Fake_http.last_request) with
  | Some request -> assert_contains "/vector_stores/vs_123/file_batches" request.url
