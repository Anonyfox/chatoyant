...
module Fake_http = struct
  ...
  let next_response_body =
    ref
      "{\"id\":\"chatcmpl_compat\",\"model\":\"local-model\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"Hello compatible\"},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":3,\"completion_tokens\":2,\"total_tokens\":5}}"

  let send request =
    last_request := Some request;
    Ok { status = !next_response_status; headers = []; body = !next_response_body }
end
...
let test_local_normalization () =
  let body =
    request_fixture ()
    |> Chatoyant.Provider.Local.chat_request_json
...
let test_clients () =
  ...
  | Ok response -> assert_equal_string "Hello compatible" response.chat_response_content);
  ...
  Fake_http.next_response_body :=
    "{\"id\":\"chatcmpl_or\",\"model\":\"anthropic/claude-sonnet-4.5\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"Hello OpenRouter\"},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":1,\"completion_tokens\":1,\"total_tokens\":2,\"cost\":2.0}}";
  ...
  in
  (match Openrouter_client.create_chat openrouter_config (request_fixture ()) with
  | Error error -> failwith error.error_message
  | Ok response ->
      assert_equal_string "Hello OpenRouter" response.chat_response_content;
      assert_equal_float 0.000002 (Option.get response.chat_response_usage.actual_cost_usd));
  match !(Fake_http.last_request) with
  | None -> failwith "expected OpenRouter request"
  | Some request ->
      assert_contains "https://openrouter.ai/api/v1/chat/completions" request.url;
      if not (List.mem ("HTTP-Referer", "https://example.com") request.headers) then
        failwith "missing OpenRouter referer";
...
let test_local_profile_endpoints () =
  ...
  Fake_http.next_response_body :=
    "{\"id\":\"resp_local\",\"model\":\"local-responses-model\",\"status\":\"completed\",\"output_text\":\"Local Responses ok\",\"usage\":{\"input_tokens\":1,\"output_tokens\":2,\"total_tokens\":3}}";
  (match Local_client.create_response local_config response_request with
  | Error error -> failwith error.error_message
  | Ok response -> assert_equal_string "Local Responses ok" response.responses_output_text);
  (match !(Fake_http.last_request) with
  | Some request ->
...
let test_openrouter_extra_endpoints () =
  ...
  Fake_http.next_response_body :=
    "{\"id\":\"resp_123\",\"model\":\"openai/gpt-4o-mini\",\"status\":\"completed\",\"output_text\":\"OpenRouter Responses ok\",\"usage\":{\"input_tokens\":1,\"output_tokens\":2,\"total_tokens\":3}}";
  (match
   ...
   with
  | Error error -> failwith error.error_message
  | Ok response -> assert_equal_string "OpenRouter Responses ok" response.responses_output_text);
  (match !(Fake_http.last_request) with
  | Some request ->
  ...
  (match Openrouter_client.retrieve_generation config ~generation_id:"gen_123" with
  | Error error -> failwith error.error_message
  | Ok generation -> assert_equal_string "gen_123" (Option.get generation.generation_id));
  Fake_http.next_response_body := "{\"data\":{\"count\":150}}";
  (match Openrouter_client.count_models ~output_modalities:"text,image" config with
  | Error error -> failwith error.error_message
  | Ok count -> assert_equal_int 150 count.model_count);
  (match !(Fake_http.last_request) with
  | Some request -> assert_contains "/models/count?output_modalities=text,image" request.url
  ...
  let endpoint_fixture =
    "{\"data\":{\"id\":\"openai/gpt-4o-mini\",\"name\":\"GPT-4o mini\",\"description\":\"Fast small model\",\"created\":1700000000,\"architecture\":{\"modality\":\"text->text\"},\"endpoints\":[{\"name\":\"openai/gpt-4o-mini\",\"provider_name\":\"OpenAI\",\"context_length\":128000,\"max_completion_tokens\":16384,\"supported_parameters\":[\"tools\",\"response_format\"],\"pricing\":{\"prompt\":\"0.00000015\"},\"status\":\"stable\"}]}}"
  in
  let endpoint_list =
    Chatoyant.Provider.Openrouter.model_endpoint_list_of_json
      ...
      | Error message -> failwith message)
  in
  assert_equal_string "openai/gpt-4o-mini" (Option.get endpoint_list.model_endpoint_model_id);
  assert_equal_int 1 (List.length endpoint_list.model_endpoints);
  assert_equal_int 128000
    (Option.get (List.hd endpoint_list.model_endpoints).model_endpoint_context_length);
  Fake_http.next_response_body := endpoint_fixture;
  (match Openrouter_client.list_model_endpoints config ~author:"openai" ~slug:"gpt-4o-mini" with
  | Error error -> failwith error.error_message
  | Ok endpoints -> assert_equal_string "GPT-4o mini" (Option.get endpoints.model_endpoint_model_name));
  (match !(Fake_http.last_request) with
  | Some request ->
  ...
  (match Openrouter_client.list_model_endpoints_by_id config ~model_id:"openai/gpt-4o-mini" with
  | Error error -> failwith error.error_message
  | Ok endpoints -> assert_equal_int 1 (List.length endpoints.model_endpoints));
  (match Openrouter_client.list_model_endpoints_by_id config ~model_id:"bad-model-id" with
    ...
    Chatoyant.Provider.Openrouter.
      {
        ...
        rerank_documents =
          [
            Rerank_text "first";
            Rerank_object (Chatoyant.Runtime.Json.Object [ ("text", Chatoyant.Runtime.Json.String "second") ]);
          ];
        rerank_top_n = Some 1;
        rerank_provider = Some (Chatoyant.Runtime.Json.Object [ ("order", Chatoyant.Runtime.Json.Array []) ]);
        rerank_extra = [];
      }
  ...
  Fake_http.next_response_body :=
    "{\"data\":[{\"id\":\"google/veo-3.1\",\"name\":\"Veo 3.1\",\"canonical_slug\":\"google/veo-3.1\",\"created\":1700000000}]}";
...
let test_openrouter_management_endpoints () =
  ...
  | Some request ->
      ...
      if not (List.mem ("Authorization", "Bearer management-key") request.headers) then
        failwith "missing OpenRouter management key"
  | None -> failwith "expected OpenRouter list keys request");
  Fake_http.next_response_body :=
    "{\"data\":{\"hash\":\"key_hash_123\",\"name\":\"prod key\"}}";
  (match
     Openrouter_client.update_key management ~key_hash:"key_hash_123"
       (Chatoyant.Runtime.Json.Object [ ("name", Chatoyant.Runtime.Json.String "prod key") ])
   with
  | Error error -> failwith error.error_message
  ...
  Fake_http.next_response_body :=
    "{\"id\":\"guardrail_123\",\"deleted\":true}";
  match Openrouter_client.delete_guardrail management ~guardrail_id:"guardrail_123" with
  | Error error -> failwith error.error_message
  | Ok deleted ->
      assert_equal_string "guardrail_123" (Option.get deleted.management_delete_id);
      if not deleted.management_deleted then failwith "expected guardrail delete flag"

let test_provider_adapter () =
  let module Local_provider = Chatoyant.Provider.Local.Make_provider (Fake_http) (struct
  ...
  Fake_http.next_response_body :=
    "{\"id\":\"chatcmpl_local\",\"model\":\"local-model\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"Adapter ok\"},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":1,\"completion_tokens\":1,\"total_tokens\":2}}";
  match
