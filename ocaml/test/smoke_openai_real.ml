module Curl_http = struct
  ...
  let send (request : request) =
    ...
    let response_path = Filename.temp_file "chatoyant-openai-response" ".json" in
    ...
    Chatoyant.Provider.Openai.
      {
        ...
        responses_input =
          Input_text "Respond with the exact text OCAML_OPENAI_RESPONSES_OK and nothing else.";
          ...
          [
            {
              message_role = User;
              message_content =
                Some "Respond with the exact text OCAML_OPENAI_CHAT_OK and nothing else.";
              message_name = None;
