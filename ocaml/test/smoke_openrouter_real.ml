module Curl_http = struct
  ...
  let send (request : request) =
    ...
    let response_path = Filename.temp_file "chatoyant-openrouter-response" ".json" in
...
let () =
  let api_key =
    ...
    | _ -> failwith "OPENROUTER_API_KEY is required for real OpenRouter smoke test"
  in
  let request =
          ...
          [
            {
              message_role = User;
              message_content =
                Some "Respond with the exact text OCAML_OPENROUTER_OK and nothing else.";
              message_name = None;
