...
module%tool Describe_city = struct
  ...
  type request = {
    location : location; (** Location to describe. *)
  }
...
let test_direct_run () =
  match
    Calculate.run
      { operation = Divide; values = [ 83.; 3. ]; precision = None }
  with
  | Error message -> fail ("unexpected direct run error: " ^ message)
  | Ok answer -> assert_equal_float ~message:"direct result" (83. /. 3.) answer.result
...
let test_generated_type_helpers () =
  ...
  | Ok request ->
      if request.Calculate.operation <> Divide then fail "helper decoded wrong operation";
      if request.Calculate.precision <> Some 3 then fail "helper decoded wrong option";
      let _schema_json = Chatoyant.Schema.Schema.to_json_schema Calculate.request_schema in
      ()
...
let test_deriving_chatoyant_data () =
  ...
  let schema_json =
    Chatoyant.Schema.Schema.to_json_schema invoice_schema
  in
  let properties =
