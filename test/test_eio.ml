...
let test_http_json env =
  ...
  with_server env
    (fun request body ->
      seen := true;
      assert_equal_string "POST"
        (request |> Http.Request.meth |> Http.Method.to_string);
      assert_equal_string "/json" (Http.Request.resource request);
      let content_type =
        request |> Http.Request.headers |> fun headers ->
        Http.Header.get headers "content-type"
      in
      assert_equal_string "application/json"
    ...
    (fun base_url ->
      ...
      | Error (Timeout ms) ->
          failwith (Printf.sprintf "unexpected timeout after %d ms" ms)
      | Error (Network message) | Error (Invalid_response message) ->
          failwith message);
  ...
  with_server env
    (fun request body ->
      let content_type =
        ...
        Http.Header.get headers "content-type"
      in
      assert_contains "multipart/form-data; boundary="
        (Option.value content_type ~default:"");
  ...
  with_server env
    ...
    (fun base_url ->
      ...
      | Ok json -> (
          ...
          | Some (Chatoyant.Runtime.Json.Float value) ->
              assert_equal_int 12 (int_of_float value)
...
let openai_response =
  "{\"id\":\"resp_eio\",\"object\":\"response\",\"model\":\"gpt-4o-mini\",\"status\":\"completed\",\"output\":[{\"type\":\"message\",\"content\":[{\"type\":\"output_text\",\"text\":\"Eio \
   provider \
   ok\"}]}],\"usage\":{\"input_tokens\":5,\"output_tokens\":4,\"total_tokens\":9}}"
...
let text_shortcut_response =
  "{\"id\":\"resp_text\",\"object\":\"response\",\"model\":\"gpt-4o-mini\",\"status\":\"completed\",\"output\":[{\"type\":\"message\",\"content\":[{\"type\":\"output_text\",\"text\":\"shortcut \
   text \
   ok\"}]}],\"usage\":{\"input_tokens\":3,\"output_tokens\":3,\"total_tokens\":6}}"
...
let test_top_level_shortcuts env =
  ...
  with_server env
    ...
    (fun base_url ->
      let ai =
        Chatoyant.openai ~https:Chatoyant.Http.Disabled env ~api_key:"test-key"
      ...
      | Error error -> failwith (Chatoyant.Error.provider error));
      (match
         [%chatoyant.gen_data: extracted_answer] ai "Extract the answer"
       with
      | Ok answer ->
