module Curl_http = struct
  ...
  let body_to_string = function
    ...
    | Multipart _ ->
        failwith "multipart body is not supported by this smoke transport"
  ...
  let config_for (request : request) body_path response_path =
    ...
    let headers =
      ...
      |> List.map (fun (name, value) ->
          Printf.sprintf "header = %S" (name ^ ": " ^ value))
    in
    String.concat "\n"
      ([
  ...
  let send (request : request) =
    let body_path = Filename.temp_file "chatoyant-openai-body" ".json" in
    let config_path = Filename.temp_file "chatoyant-openai-curl" ".conf" in
    let response_path =
      Filename.temp_file "chatoyant-openai-response" ".json"
    in
    let error_path = Filename.temp_file "chatoyant-openai-error" ".txt" in
    Fun.protect
      ~finally:(fun () ->
        List.iter
      ...
      (fun () ->
        ...
        let command =
          "curl --config " ^ shell_quote config_path ^ " 2> "
          ^ shell_quote error_path
        in
        let ic = Unix.open_process_in command in
        let status_text = try input_line ic with End_of_file -> "" in
        ...
        | Unix.WEXITED 0 -> (
            ...
            | None ->
                Error
                  (Invalid_response
                     ("curl did not return HTTP status: " ^ status_text))
            | Some status ->
                Ok { status; headers = []; body = read_file response_path })
        | _ -> Error (Network (read_file error_path)))
end

module Client = Chatoyant.Provider.Openai.Make_client (Curl_http)

let () =
  let api_key =
    match Sys.getenv_opt "OPENAI_API_KEY" with
    | Some value when value <> "" -> value
    | _ -> failwith "OPENAI_API_KEY is required for real OpenAI smoke test"
  in
  let config =
    ...
    Chatoyant.Provider.Openai.
      {
        ...
        responses_input =
          Input_text
            "Respond with the exact text OCAML_OPENAI_RESPONSES_OK and nothing \
             else.";
  ...
  | Ok response ->
      ...
      if not (String.equal text "OCAML_OPENAI_RESPONSES_OK") then
        failwith ("unexpected OpenAI Responses smoke response: " ^ text));
  let chat_request =
    Chatoyant.Provider.Openai.
      {
          ...
          [
            {
              ...
              message_content =
                Some
                  "Respond with the exact text OCAML_OPENAI_CHAT_OK and \
                   nothing else.";
              message_name = None;
  ...
  in
  match Client.create_chat config chat_request with
  | Error error -> failwith error.error_message
  | Ok response ->
      let text = String.trim response.chat_response_content in
      if not (String.equal text "OCAML_OPENAI_CHAT_OK") then
        failwith ("unexpected OpenAI Chat smoke response: " ^ text);
      print_endline "ocaml openai smoke ok"
