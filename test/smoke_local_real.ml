module Curl_http = struct
  ...
  let send (request : request) =
    let body_path = Filename.temp_file "chatoyant-local-body" ".json" in
    let config_path = Filename.temp_file "chatoyant-local-curl" ".conf" in
    let response_path = Filename.temp_file "chatoyant-local-response" ".json" in
    let error_path = Filename.temp_file "chatoyant-local-error" ".txt" in
    Fun.protect
      ~finally:(fun () ->
        List.iter
...
module Client = Chatoyant.Provider.Local.Make_client (Curl_http)

let required_env name =
  match Sys.getenv_opt name with
  | Some value when value <> "" -> value
  | _ -> failwith (name ^ " is required for real Local smoke test")

let () =
  let base_url = required_env "LOCAL_BASE_URL" in
  let model = required_env "LOCAL_MODEL" in
  let config =
    Client.
      {
        base_url;
        api_key = Sys.getenv_opt "LOCAL_API_KEY";
          ...
          [
            {
              ...
              message_content =
                Some
                  "Respond with the exact text OCAML_LOCAL_OK and nothing else.";
              message_name = None;
  ...
  | Ok response ->
      if String.trim response.chat_response_content <> "OCAML_LOCAL_OK" then
        failwith ("unexpected local response: " ^ response.chat_response_content);
      print_endline "ocaml local real smoke ok"
