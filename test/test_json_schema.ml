...
let assert_valid ?options schema data =
  ...
  if not result.valid then
    failwith
      ("expected valid, got "
      ^ String.concat "; "
          (List.map Chatoyant.Schema.Json_schema.Output.error_to_string
             result.errors))
...
type search_args = { q : string; limit : int option }
...
let test_typed_codecs_and_tools () =
  ...
  let args_codec : search_args Codec.t =
    Codec.object2
      (Codec.field "q" (Codec.string ~description:"Search query." ())
         ~get:(fun value -> value.q))
      (Codec.field "limit"
         (Codec.optional (Codec.int ()))
         ~get:(fun value -> value.limit))
      (fun q limit -> { q; limit })
  in
  ...
  in
  let schema_json =
    Codec.to_json_schema args_codec |> Chatoyant.Runtime.Json.to_string
  in
  if not (String.contains schema_json 'q') then
    failwith "codec schema missing field";
...
let () =
  ...
  assert_valid {|{"type":["string","null"],"minLength":2}|} {|"ok"|};
  assert_valid {|{"type":["string","null"],"minLength":2}|} {|null|};
  assert_invalid {|{"type":["string","null"],"minLength":2}|} {|"x"|};
  ...
  assert_invalid {|{"dependentRequired":{"credit_card":["billing_address"]}}|}
    {|{"credit_card":1}|};
  ...
  in
  if not (String.contains projected_json 'f') then
    failwith "projection did not encode";
...
let () =
  ...
  assert_valid ~options:external_options
    {|{"$ref":"http://example.com/integer.json"}|} {|1|};
  assert_invalid ~options:external_options
    {|{"$ref":"http://example.com/integer.json"}|} {|"x"|};
