module Curl_http = struct
  ...
  let send (request : request) =
    let body_path = Filename.temp_file "chatoyant-openrouter-body" ".json" in
    let config_path = Filename.temp_file "chatoyant-openrouter-curl" ".conf" in
    let response_path =
      Filename.temp_file "chatoyant-openrouter-response" ".json"
    in
    let error_path = Filename.temp_file "chatoyant-openrouter-error" ".txt" in
    Fun.protect
      ~finally:(fun () ->
        List.iter
...
module Client = Chatoyant.Provider.Openrouter.Make_client (Curl_http)
...
let () =
  let api_key =
    ...
    | _ ->
        failwith "OPENROUTER_API_KEY is required for real OpenRouter smoke test"
  in
  let request =
          ...
          [
            {
              ...
              message_content =
                Some
                  "Respond with the exact text OCAML_OPENROUTER_OK and nothing \
                   else.";
              message_name = None;
    ...
    Client.
      {
        ...
        http_referer = None;
        title = Some "Chatoyant OCaml smoke";
        headers = [];
      }
  in
  ...
  | Ok response ->
      ...
      if not (String.equal text "OCAML_OPENROUTER_OK") then
        failwith ("unexpected OpenRouter smoke response: " ^ text);
      print_endline "ocaml openrouter smoke ok"
