...
module Fake_provider : Chatoyant.Provider.Provider.CHAT = struct
  ...
  let generate messages options =
    if messages = [] then
      Error (Chatoyant.Provider.Provider.Runtime_error "missing messages")
    else
      Ok
        {
...
module Fake_tool_provider : Chatoyant.Provider.Provider.CHAT = struct
  ...
  let generate messages options =
    if options.Chatoyant.Provider.Provider.tools = [] then
      Error
        (Chatoyant.Provider.Provider.Runtime_error "missing tool definitions")
...
module Fake_generator =
  Chatoyant.Core.Generator.Make (Fake_provider) (Fake_clock)

module Fake_session = Chatoyant.Core.Session.Make (Fake_provider) (Fake_clock)

module Fake_shortcuts =
  Chatoyant.Core.Shortcuts.Make (Fake_provider) (Fake_clock)

module Fake_tool_session =
  Chatoyant.Core.Session.Make (Fake_tool_provider) (Fake_clock)
    ...
    Ok
      {
        ...
        body =
          "{\"id\":\"msg_1\",\"type\":\"message\",\"role\":\"assistant\",\"model\":\"claude-sonnet-4-6\",\"content\":[{\"type\":\"text\",\"text\":\"Hello \
           from \
           Claude\"}],\"stop_reason\":\"end_turn\",\"usage\":{\"input_tokens\":7,\"output_tokens\":3}}";
      }
end

module Anthropic_client =
  Chatoyant.Provider.Anthropic.Make_client (Fake_anthropic_http)

let () =
  assert_equal_int 4 (Chatoyant.Tokens.Token_estimate.estimate "Hello, world!");
  let prompt_estimate =
    Chatoyant.Tokens.Token_estimate.estimate_prompt ~response:"Hi" "Hello"
  in
  assert_equal_int
    (prompt_estimate.input + prompt_estimate.output)
    prompt_estimate.total;
  assert_equal_int 128_000
    (Option.get (Chatoyant.Tokens.Context_window.get "gpt-4o"));
  assert_equal_int 131_072
    (Option.get (Chatoyant.Tokens.Context_window.get "grok-3"));
  if not (Chatoyant.Tokens.Pricing.has "gpt-5.4-mini") then
    failwith "missing pricing";
  if Chatoyant.Tokens.Pricing.has "gpt-99" then
    failwith "fallback pricing should not be explicit";
  ...
  let custom_pricing =
    Chatoyant.Tokens.Cost.pricing ~input_per_million:10.0
      ~output_per_million:20.0 ~cache_write_per_million:12.5 ()
  in
  let custom_cost =
  ...
  let batch_cost =
    Chatoyant.Tokens.Cost.calculate_batch
      ~pricing:(Chatoyant.Tokens.Pricing.get "gpt-4o")
      [
        {
          Chatoyant.Tokens.Cost.empty_usage with
  ...
  let messages =
    [
      {
        Chatoyant.Tokens.Message_budget.role = "system";
  ...
  let chunks =
    Chatoyant.Tokens.Chunking.split_text ~max_tokens:10
      ("Word " ^ String.make 200 'a')
  in
  if chunks = [] then failwith "expected chunks";
  let pages =
    Chatoyant.Tokens.Chunking.paginate_messages ~tokens_per_page:20 messages
  in
  if pages = [] then failwith "expected message pages";
  let truncated =
    Chatoyant.Tokens.Chunking.truncate_content ~max_tokens:1 "Hello world"
  in
    ...
    Chatoyant.Core.Options.merge
      {
        ...
        extra =
          Some
            (Chatoyant.Runtime.Json.Object
               [ ("a", Chatoyant.Runtime.Json.Float 1.0) ]);
      }
      {
        Chatoyant.Core.Options.default with
        top_p = Some 0.9;
  ...
  let sse_state, events =
    Chatoyant.Runtime.Sse.feed sse_state "1}\n\ndata: [DONE]\n\n"
  in
  ignore sse_state;
  assert_equal_int 2 (List.length events);
  assert_contains "{\"a\":1}"
    (Chatoyant.Runtime.Sse.data_string (List.hd events));
  if not (Chatoyant.Runtime.Sse.is_done (List.nth events 1)) then
    failwith "expected done event";
  ...
  | Ok result ->
      ...
      let result_json =
        Chatoyant.Core.Result.generation_to_json result
  ...
  | Ok result ->
      ...
      assert_equal_string "fake"
        (Option.get (Fake_session.last_result session)).content);
  let session_json =
    Fake_session.to_json session |> Chatoyant.Runtime.Json.to_string
  in
  ...
  if Fake_session.last_result forked <> None then
    failwith "fork should clear last result";
  let cloned = Fake_session.clone session in
  assert_equal_int
    (List.length (Fake_session.messages session))
    (List.length (Fake_session.messages cloned));
  (match
     Fake_shortcuts.gen_text ~system:"You are concise" ~model:"gpt-4o" "Hello"
   with
  | Error _ -> failwith "shortcut gen_text failed"
  | Ok text -> assert_equal_string "fake" text);
  let tool_params =
    Chatoyant.Schema.Schema.object_
      [ ("city", Chatoyant.Schema.Schema.string ()) ]
  in
  let weather_tool =
    Tool.create ~name:"weather" ~description:"Get weather"
      ~parameters:tool_params (fun _ctx args -> Ok args)
  in
  ...
  let tool_result_json =
    Tool.result_to_json tool_result |> Chatoyant.Runtime.Json.to_string
  in
  ...
  | Ok result -> (
      ...
      | {
       Chatoyant.Core.Message.role = Tool;
       tool_result_error = Some false;
       content;
       _;
      } ->
  ...
  let message_json =
    Chatoyant.Core.Message.to_json
      ( Chatoyant.Core.Message.assistant "Use a tool" |> fun message ->
        {
          message with
          tool_calls =
            [
  ...
  let bad_tool_result =
    Tool.execute_call
      { model = "gpt-4o"; provider = Chatoyant.Provider.Provider.Openai }
      {
        id = "call_2";
        name = "weather";
        arguments = Chatoyant.Runtime.Json.Object [];
      }
      weather_tool
  in
  ...
  in
  let json =
    Chatoyant.Schema.Schema.to_json_schema schema
    |> Chatoyant.Runtime.Json.to_string
  in
  if not (String.contains json '{') then
    failwith "schema did not encode to an object";
  let valid_user =
    Chatoyant.Runtime.Json.Object
      [
  ...
  | Error error ->
      assert_contains "age" (Chatoyant.Schema.Value.error_to_string error));

  assert_equal_provider Chatoyant.Provider.Provider.Openrouter
    (Option.get
       (Chatoyant.Provider.Registry.detect_by_model "anthropic/claude-opus-4"));
  assert_equal_provider Chatoyant.Provider.Provider.Openai
    (Option.get (Chatoyant.Provider.Registry.detect_by_model "gpt-4o"));
  assert_equal_provider Chatoyant.Provider.Provider.Anthropic
    (Option.get
       (Chatoyant.Provider.Registry.detect_by_model "claude-sonnet-4-6"));
  assert_equal_provider Chatoyant.Provider.Provider.Xai
    (Option.get (Chatoyant.Provider.Registry.detect_by_model "grok-4"));
  assert_equal_provider Chatoyant.Provider.Provider.Local
    (Option.get
                  ...
                  ( "delta",
                    Chatoyant.Runtime.Json.Object
                      [
                        ...
                        ( "reasoning_content",
                          Chatoyant.Runtime.Json.String "Think" );
                      ] );
            ...
            [
              Chatoyant.Runtime.Json.Object
                [
                  ( "delta",
                    Chatoyant.Runtime.Json.Object
                      [ ("content", Chatoyant.Runtime.Json.String "lo") ] );
                  ("finish_reason", Chatoyant.Runtime.Json.String "stop");
                ];
            ] );
        ( "usage",
          Chatoyant.Runtime.Json.Object
            [ ("total_tokens", Chatoyant.Runtime.Json.Float 3.0) ] );
      ]
  in
  let accumulated =
    Chatoyant.Provider.Openai_stream.empty |> fun acc ->
    Chatoyant.Provider.Openai_stream.apply_chunk_json acc openai_chunk
    |> fun acc ->
    Chatoyant.Provider.Openai_stream.apply_chunk_json acc openai_done_chunk
  in
  ...
  in
  assert_equal_string "decoded chat"
    (Option.get
       (Chatoyant.Provider.Openai_decode.chat_content chat_response_json));
  let responses_json =
    Chatoyant.Runtime.Json.Object
      [
  ...
  in
  assert_equal_string "decoded response"
  ...
  in
  if Option.get xai_usage.actual_cost_usd <> 1.0 then
    failwith "bad xAI cost ticks";
  ...
  let usage_json =
    Chatoyant.Tokens.Cost.usage_to_json normalized_usage
    |> Chatoyant.Runtime.Json.to_string
  in
  ...
  assert_equal_string "Thinking"
    (Chatoyant.Core.Stream.reasoning_content stream_state);
  let unified_tool_call =
    List.hd (Chatoyant.Core.Stream.tool_calls stream_state)
  in
  ...
  let streamed_generation =
    Chatoyant.Core.Stream.to_generation
      ~provider:Chatoyant.Provider.Provider.Openai ~model:"gpt-4o"
      ~started_ms:100 ~finished_ms:600 stream_state
  in
  assert_equal_int 25
    (Option.get streamed_generation.timing.time_to_first_token_ms);
  ...
  assert_equal_string "tool_calls"
    (Option.get streamed_generation.finish_reason);
  let stream_json =
    Chatoyant.Core.Stream.state_to_json stream_state
    |> Chatoyant.Runtime.Json.to_string
  in
  assert_contains "\"usage_source\":\"provider_reported\"" stream_json;
  assert_contains "\"arguments_json\":\"{\\\"city\\\":\\\"Berlin\\\"}\""
    stream_json;
            ...
            [
              ...
              Chatoyant.Runtime.Json.Object
                [
                  ...
                  ( "input",
                    Chatoyant.Runtime.Json.Object
                      [ ("query", Chatoyant.Runtime.Json.String "x") ] );
                ];
            ] );
  ...
  let stream_state =
    List.fold_left
      (fun state sse ->
        ...
        | Error message -> failwith message
        | Ok event ->
            Chatoyant.Provider.Anthropic.apply_stream_event state event)
      Chatoyant.Provider.Anthropic.empty_stream_state
      [ start_sse; block_sse; delta_sse; stop_sse ]
  in
  let stream_response =
    Chatoyant.Provider.Anthropic.stream_state_to_response stream_state
  in
  ...
  | Ok response -> (
      ...
      | Chatoyant.Provider.Anthropic.Text text :: _ ->
          assert_equal_string "Hello" text
  ...
  let module Anthropic_provider =
    Chatoyant.Provider.Anthropic.Make_provider
      (Fake_anthropic_http)
      (struct
  ...
  let openrouter_headers =
    Chatoyant.Provider.Openrouter.authorization_headers ~api_key:"key"
      ~http_referer:"https://example.com" ~title:"Chatoyant" ()
  in
