...
let test_request_json () =
  let body =
    request_fixture () |> Chatoyant.Provider.Anthropic.request_json
...
let test_cache_control_json () =
  ...
  let cache_1h =
    Chatoyant.Provider.Anthropic.ephemeral_cache_control ~ttl:Ttl_1h ()
  in
  let request =
    Chatoyant.Provider.Anthropic.
      {
        ...
        system_blocks =
          [
            Cached_block
              { block = Text "Reusable policy"; cache_control = cache_5m };
          ];
        messages =
          [
            {
              ...
              message_content =
                [
                  Cached_block
                    {
                      block = Text "Large reusable context";
  ...
  in
  let body =
    Chatoyant.Provider.Anthropic.request_json request
    |> Chatoyant.Runtime.Json.to_string
  in
  assert_contains "\"cache_control\":{\"type\":\"ephemeral\",\"ttl\":\"1h\"}"
    body;
  assert_contains
    "\"system\":[{\"type\":\"text\",\"text\":\"Stable system instruction\"}"
    body;
  assert_contains
    "\"cache_control\":{\"type\":\"ephemeral\"},\"type\":\"text\",\"text\":\"Reusable \
     policy\""
    body;
  assert_contains
    "\"cache_control\":{\"type\":\"ephemeral\",\"ttl\":\"1h\"},\"type\":\"text\",\"text\":\"Large \
     reusable context\""
    body;
...
let test_server_tools_and_admin_helpers () =
  ...
  let raw_tools =
    [
      Chatoyant.Provider.Anthropic.web_search_tool_json ~cache_control
        ~max_uses:2 ~allowed_domains:[ "example.com" ] ();
      Chatoyant.Provider.Anthropic.code_execution_tool_json ~cache_control ();
    ]
  in
  let body =
    Chatoyant.Provider.Anthropic.request_json_with_raw_tools
      (request_fixture ()) raw_tools
    |> Chatoyant.Runtime.Json.to_string
  in
               ...
               [
                 Chatoyant.Runtime.Json.Object
                   [
                     ...
                     ( "input",
                       Chatoyant.Runtime.Json.Object
                         [ ("query", Chatoyant.Runtime.Json.String "ocaml") ] );
                   ];
                 Chatoyant.Runtime.Json.Object
                   [
                     ( "type",
                       Chatoyant.Runtime.Json.String "web_search_tool_result" );
                 ...
                 Chatoyant.Runtime.Json.Object
                   [
                     ...
                     ( "content",
                       Chatoyant.Runtime.Json.Object
                         [ ("stdout", Chatoyant.Runtime.Json.String "4") ] );
                   ];
                 Chatoyant.Runtime.Json.Object
                   [
  ...
  | [
   ...
   Chatoyant.Provider.Anthropic.Container_upload
     { file_id = Some "file_123"; _ };
  ] ->
      ()
...
let test_response_decode () =
  ...
  assert_equal_string "Hello"
    (Chatoyant.Provider.Anthropic.text_of_response response);
...
let test_models_and_batches_decode () =
             ...
             Chatoyant.Runtime.Json.Array
               [
                 Chatoyant.Runtime.Json.Object
                   [
                     ( "id",
                       Chatoyant.Runtime.Json.String "claude-haiku-4-5-20251001"
                     );
                     ("type", Chatoyant.Runtime.Json.String "model");
                     ( "display_name",
                       Chatoyant.Runtime.Json.String "Claude Haiku 4.5" );
                     ( "created_at",
                       Chatoyant.Runtime.Json.String "2025-10-01T00:00:00Z" );
                   ];
               ] );
  ...
  assert_equal_string "claude-haiku-4-5-20251001"
    (List.hd models.models).model_id;
  let batch =
...
let test_client_success () =
  ...
  | Ok response -> (
      assert_equal_string "Hello from Claude"
        (Chatoyant.Provider.Anthropic.text_of_response response);
...
let test_client_models_and_batches () =
  ...
  in
  Fake_http.next_response_body :=
    "{\"data\":[{\"id\":\"claude-haiku-4-5-20251001\",\"type\":\"model\",\"display_name\":\"Claude \
     Haiku 4.5\",\"created_at\":\"2025-10-01T00:00:00Z\"}],\"has_more\":false}";
  ...
  (match
     Client.create_message_batch config
       [ { custom_id = "case_1"; params = request_fixture () } ]
   with
  | Error error -> failwith error.error_message
...
let test_client_files () =
  ...
  | Some request -> (
      ...
      if
        not
          (List.mem ("anthropic-beta", "files-api-2025-04-14") request.headers)
      then failwith "missing files beta header";
...
let test_client_admin_reports () =
  ...
  (match
     Client.get_usage_report_messages admin ~starting_at:"2026-06-01T00:00:00Z"
       ~ending_at:"2026-06-02T00:00:00Z" ~bucket_width:"1d"
       ~group_by:[ "model" ] ()
   with
  | Error error -> failwith error.error_message
  | Ok json ->
