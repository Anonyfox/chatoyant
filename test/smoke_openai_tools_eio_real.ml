...
module%tool Lookup_city_metric = struct
  ...
  type request = { city : string  (** City name, for example Berlin. *) }
  type metric = { city : string; metric : float; source : string }

  (** Look up a city metric by making a real HTTP call from inside the tool
      handler. *)
  let run : ('net, 'clock) env -> request -> (metric, string) result =
   fun { net; clock; base_url } { city } ->
    ...
    match
      Eio_http.get_json ~https:Eio_http.Disabled ~net ~clock ~timeout_ms:5_000
        url
    with
  ...
  let server =
    Cohttp_eio.Server.make
      ~callback:(fun _ request _body ->
        let uri =
          Uri.of_string ("http://localhost" ^ Http.Request.resource request)
        in
        let city =
          Option.value (Uri.get_query_param uri "city") ~default:"unknown"
        in
        let status, body =
...
let check_direct_tools metric_env =
  (match
     Calculate.run { Calculate.operation = Divide; values = [ 10.; 2. ] }
   with
  | Ok answer ->
      assert_close "direct calculator result" 5. answer.Calculate.result
  | Error message -> failwith ("direct calculator failed: " ^ message));
  match
    Lookup_city_metric.run metric_env { Lookup_city_metric.city = "Hamburg" }
  with
  | Ok metric ->
...
let run env =
  let model =
    Option.value (Sys.getenv_opt "MODEL_OPENAI") ~default:"gpt-5.4-mini"
  in
  ...
  match
    Chat.ask ~tools ~timeout_ms:120_000 ~max_tokens:1_200
      ~system:
        "Use tools whenever arithmetic or city lookup is needed. For this \
         task, call both available tools before answering. Return a concise \
         final answer."
      "Use the calculator for 83 divided by 3. Also look up the city metric \
       for Berlin through the HTTP-backed tool. Then use the calculator again \
       to multiply the quotient by the metric. Show both calculations and name \
       the metric source."
      chat
  with
  | Error error ->
  ...
  | Ok result ->
      ...
      let assistant_tool_calls =
        List.fold_left
          (fun count message ->
            count + List.length message.Chatoyant.Core.Message.tool_calls)
          0 messages
      in
      let tool_results =
        messages
        |> List.filter (fun message ->
            message.Chatoyant.Core.Message.role = Tool)
        |> List.length
      in
