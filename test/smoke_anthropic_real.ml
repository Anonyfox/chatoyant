module Curl_http = struct
  ...
  let send (request : request) =
    ...
    let response_path =
      Filename.temp_file "chatoyant-anthropic-response" ".json"
    in
...
let () =
  let api_key =
    ...
    | _ ->
        failwith "ANTHROPIC_API_KEY is required for real Anthropic smoke test"
  in
  let request =
          ...
          [
            {
              ...
              message_content =
                [
                  Text
                    "Respond with the exact text OCAML_SMOKE_OK and nothing \
                     else.";
                ];
            };
          ];
