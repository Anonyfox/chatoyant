...
`make check` builds and tests the native OCaml package first, emits Melange ESM,
bundles the npm package with esbuild, runs Node's native tests against
`dist/index.js`, checks `dist/index.d.ts` with `tsc`, runs the pinned official
JSON Schema suite through the bundled package, lints opam metadata, checks local
documentation links, verifies package metadata, and dry-packs the npm artifact.
